//+------------------------------------------------------------------+
//|                  TORAMA_MomentumGrid_TrueSacrosanct_v3.06.mq5    |
//|                                          TORAMA CAPITAL           |
//+------------------------------------------------------------------+
#property copyright "TORAMA CAPITAL"
#property link      "https://toramacapital.com"
#property version   "3.06"
#property description "Momentum Grid EA - TRUE Sacrosanct - PRODUCTION"
#property description "✓ Position Preservation | ✓ Settings-Change Safe | ✓ Daily Profit Target"
...
int OnInit()
{
   ...
   // (Fresh start vs settings change mid-trade)
   ...
   PrintFormat("╔════════════════════════════════════════════════════════════════╗");
   PrintFormat("║  TORAMA Grid v3.06 COMPLETE - Magic: %I64d", magicNumber);
   PrintFormat("║  ✓ Reversal Threshold | ✓ Daily Profit Target | ✓ Debug P/L");
   ...
   PrintFormat("║  Daily Target: %s | Lot: %.2f | Gap: %.2f%%", 
               InpDailyProfitTargetPercent > 0 ? StringFormat("%.1f%%", InpDailyProfitTargetPercent) : "OFF",
...
void OnTick()
{
   ...
   // If daily target is reached, stop trading
...
{
   // Check if we have existing positions from previous run
   int existingPositions = CountExistingPositions();
   
   if(existingPositions > 0)
   {
      // PRESERVE existing positions - adapt grid to them
      PrintFormat("Found %d existing positions - adapting grid to preserve them", existingPositions);
      AdaptGridToExistingPositions();
   }
   else
   {
      // No existing positions - initialize fresh grid
...
// Count existing positions with our magic number
...
// Adapt grid to existing positions instead of closing them
void AdaptGridToExistingPositions()
{
   // Find the average entry price of existing positions
   double totalPrice = 0;
   int posCount = 0;
   double lowestBuyPrice = DBL_MAX;
   double highestSellPrice = -DBL_MAX;
   ...
   for(int i = 0; i < PositionsTotal(); i++)
   {
      ...
      ENUM_POSITION_TYPE posType = (ENUM_POSITION_TYPE)PositionGetInteger(POSITION_TYPE);
      if(posType == POSITION_TYPE_BUY && entryPrice < lowestBuyPrice)
         lowestBuyPrice = entryPrice;
      if(posType == POSITION_TYPE_SELL && entryPrice > highestSellPrice)
         highestSellPrice = entryPrice;
   }
   
   if(posCount == 0) return;
   
   // Set reference price to the average of existing positions
   // This ensures all existing positions remain valid in the new grid
   double avgPrice = totalPrice / posCount;
   
   // Round to nearest grid level based on current grid gap
   referencePrice = MathRound(avgPrice / gridGapPrice) * gridGapPrice;
   referencePrice = MathRound(referencePrice / tickSize) * tickSize;
   
   PrintFormat("Grid adapted: Ref=%.5f based on %d existing positions (avg=%.5f)", 
               referencePrice, posCount, avgPrice);
   
   // Recalculate all triggered levels based on existing positions
   RecalculateTriggeredLevels();
   
   // Update highest/lowest levels
   highestBuyLevel = lowestSellLevel = 0;
   UpdateGridLevelsFromPositions();
   
   // Delete old pending orders that may not match new grid
   DeleteAllPendingOrders();
...
void CheckDailyProfitTarget()
{
   ...
   if(currentDay > lastDayCheck)
   {
      ...
      PrintFormat("╔════════════════════════════════════════════════════════════════╗");
      PrintFormat("║  NEW TRADING DAY - Daily Profit Tracking Reset");
      PrintFormat("║  Start Balance: $%.2f | Target: %.1f%% ($%.2f)", 
                  dailyStartBalance, 
                  InpDailyProfitTargetPercent,
                  dailyStartBalance * InpDailyProfitTargetPercent / 100.0);
      PrintFormat("╚════════════════════════════════════════════════════════════════╝");
   }
   ...
   double dailyProfitPercent = (dailyStartBalance > 0) ? (dailyProfit / dailyStartBalance) * 100.0 : 0.0;
   double targetProfit = dailyStartBalance * InpDailyProfitTargetPercent / 100.0;
   ...
   if(dailyProfitPercent >= InpDailyProfitTargetPercent)
   {
      ...
      PrintFormat("║  Start: $%.2f | Current: $%.2f", dailyStartBalance, currentBalance);
      PrintFormat("║  Daily Profit: $%.2f (%.2f%%) | Target: %.1f%% ($%.2f)",
                  dailyProfit, dailyProfitPercent, InpDailyProfitTargetPercent, targetProfit);
      PrintFormat("║  Closing all trades and stopping for the day...");
      PrintFormat("╚════════════════════════════════════════════════════════════════╝");
      
      // Close all positions and delete pending orders
      CloseAllPositionsParallel("Daily Target Reached");
      DeleteAllPendingOrders();
   }
}
...
void UpdatePanelFast()
{
   ...
   if(dailyTargetReached) { st = "TARGET REACHED"; stC = clrGold; }
   else if(isStoppedByDrawdown) { st = "STOPPED"; stC = clrRed; }
   ...
   // Daily profit display (if enabled)
   if(InpDailyProfitTargetPercent > 0)
   {
      ...
      color dailyProfitColor = clrLimeGreen;
      if(dailyTargetReached) dailyProfitColor = clrGold;
      else if(dailyProfitPercent >= InpDailyProfitTargetPercent * 0.8) dailyProfitColor = clrYellow;
      else if(dailyProfitPercent < 0) dailyProfitColor = clrRed;
      
      string dailyText = StringFormat("$%.2f (%.2f%%)", dailyProfit, dailyProfitPercent);
      if(dailyTargetReached) dailyText += " ✓";
      
      ObjSetS("Daily", dailyText);
      ObjSetC("Daily", dailyProfitColor);
      ObjSetS("DailyTgt", StringFormat("%.1f%% ($%.2f)", 
                                       InpDailyProfitTargetPercent, 
                                       dailyStartBalance * InpDailyProfitTargetPercent / 100.0));
   }
}
...
void CreatePanel()
{
   int h = InpEnableReversal ? 370 : 355;  // Increased height for daily profit
   if(InpDailyProfitTargetPercent > 0) h += 19;  // Add extra height for daily profit row
