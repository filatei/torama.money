//+------------------------------------------------------------------+
//|                                     GapTrader Pro EA v13.23      |
//|        MOMENTUM OPTIMIZED - Max Drawdown Input v13.23             |
//+------------------------------------------------------------------+
#property copyright "GapTrader Pro v13.23 MOMENTUM - TORAMA CAPITAL (Max DD Input)"
#property version   "13.23"
#property strict
...
input double GapSize = 30.0;  // OPTIMIZED: 15.0 outperformed 25.0 in backtests
input double TakeProfit = 100.0, GlobalTakeProfit = 500.0, StopLoss = 0.0, GlobalSL = 0.0;
...
int OnInit() {
   ...
   Print("╔════════════════════════════════════════════════════════════════╗");
   Print("║ GapTrader Pro EA v13.23 MOMENTUM - TORAMA CAPITAL              ║");
   Print("║ MaxPositions FIXED + Panel Rebuild on Settings Change         ║");
   Print("╚════════════════════════════════════════════════════════════════╝");
   Print("Magic Number: ", MagicNumber);
   Print("Gap Size: $", DoubleToString(GapSize, 2), " (OPTIMIZED for MOMENTUM)");
   Print("");
   Print("=== TRADING MODE ===");
...
bool OpenBuy() {
   ...
   if(TakeProfit > 0) {
      double tpPoints = TakeProfit / (lot * SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_VALUE));
      tp = price + tpPoints * _Point;
   }
   
   if(StopLoss > 0) {
      double slPoints = StopLoss / (lot * SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_VALUE));
...
void RebuildGrid() {
   ...
   // Reset drawdown protection state for fresh start
   stoppedByDrawdown = false;
   peakEquity = AccountInfoDouble(ACCOUNT_EQUITY);  // Reset peak to current equity
   currentDrawdownPercent = 0;
...
bool CreatePanel() {
   ...
   CreateLabel("GT_Title", "GapTrader Pro v13.23 MOMENTUM - TORAMA (Press H to toggle)", 15, 8, 12, clrNavy);
...
void UpdatePanel() {
   ...
   ObjectSetString(0, "GT_TakeProfit", OBJPROP_TEXT, "Take Profit: $" + DoubleToString(TakeProfit, 2));
   ObjectSetString(0, "GT_GlobalTP", OBJPROP_TEXT, "Global TP: $" + DoubleToString(GlobalTakeProfit, 2));
