//+------------------------------------------------------------------+
//|                                     GapTrader Pro EA v13.27      |
//|        CRITICAL FIXES - DD Peak + Global SL v13.27               |
//+------------------------------------------------------------------+
#property copyright "GapTrader Pro v13.27 - TORAMA CAPITAL"
#property version   "13.27"
#property strict
...
void OnTick() {
   ...
   // v13.27 FIX: Separate Global TP and Global SL handling
   if(CheckGlobalTP()) {
      CloseAllProfitableTrades();
      RebuildGrid();
      Print("╔════════════════════════════════════════════════════════════════╗");
      Print("║              GLOBAL TAKE PROFIT REACHED                        ║");
      Print("╚════════════════════════════════════════════════════════════════╝");
      Print("Target: $", DoubleToString(ti.currentGapSize * GlobalTakeProfitPercent / 100.0, 2));
      Print("Profit: $", DoubleToString(ti.totalProfit, 2));
      Print("*** Profitable positions closed and grid rebuilt ***");
      return;
   }
   
   if(CheckGlobalSL()) {
      double lossAmount = ti.totalProfit;
      double globalSLDollars = ti.currentGapSize * GlobalSLPercent / 100.0;
      
      CloseAllPositions();  // Close ALL positions, not just profitable!
      ti.isActive = false;  // STOP trading!
      
      Print("╔════════════════════════════════════════════════════════════════╗");
      Print("║              GLOBAL STOP LOSS TRIGGERED                        ║");
      Print("╚════════════════════════════════════════════════════════════════╝");
      Print("Loss Limit: $", DoubleToString(globalSLDollars, 2));
      Print("Actual Loss: $", DoubleToString(-lossAmount, 2));
      Print("*** ALL POSITIONS CLOSED | EA DEACTIVATED ***");
      Print("*** To resume trading, restart EA or click REBUILD GRID ***");
      
      Alert("GapTrader EA STOPPED - Global SL reached: Loss $", DoubleToString(-lossAmount, 2));
      return;
   }
...
//+------------------------------------------------------------------+
//| CHECK GLOBAL TAKE PROFIT v13.27 FIXED                            |
//+------------------------------------------------------------------+
bool CheckGlobalTP() {
   if(GlobalTakeProfitPercent <= 0) return false;
   
   double globalTPDollars = ti.currentGapSize * GlobalTakeProfitPercent / 100.0;
   return (ti.totalProfit >= globalTPDollars);
}

//+------------------------------------------------------------------+
//| CHECK GLOBAL STOP LOSS v13.27 FIXED                              |
//+------------------------------------------------------------------+
bool CheckGlobalSL() {
   if(GlobalSLPercent <= 0) return false;
   
   double globalSLDollars = ti.currentGapSize * GlobalSLPercent / 100.0;
   return (ti.totalProfit <= -globalSLDollars);
}

//+------------------------------------------------------------------+
//| DEPRECATED: OLD COMBINED CHECK (kept for compatibility)          |
//+------------------------------------------------------------------+
bool CheckGlobalTPSL() {
   // This function is deprecated in v13.27
   // Use CheckGlobalTP() and CheckGlobalSL() separately
   return CheckGlobalTP() || CheckGlobalSL();
}
