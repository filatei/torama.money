//+------------------------------------------------------------------+
//|                              ToramaGrid_Pro_v2_COMPLETE_ROBUST.mq5 |
//|                                          TORAMA CAPITAL          |
//|                                          https://torama.money    |
//+------------------------------------------------------------------+
...
#property description "Complete Robust Bidirectional Grid with strict level enforcement"
...
int OnInit()
{
   ...
   Print("========================================");
   Print("ToramaGrid Pro v2 ROBUST COMPLETE initialized");
   ...
   Print("Grid: ", ArraySize(buyLevels), " BUY + ", ArraySize(sellLevels), " SELL levels");
   Print("========================================");
...
void OnDeinit(const int reason)
{
   ...
   Print("ToramaGrid Pro v2 ROBUST deinitialized. Total Cumulative: $", DoubleToString(cumulativeProfit, 2));
}
...
void OnTick()
{
   ...
   //--- Check for new grid entries with STRICT enforcement
   CheckGridLevelsStrict();
}
...
//+------------------------------------------------------------------+
//| Initialize grid level arrays - ONE POSITION PER LEVEL            |
//+------------------------------------------------------------------+
void InitializeGridLevels()
{
   ...
   //--- Initialize BUY levels (above reference)
   ...
   //--- Initialize SELL levels (below reference)
   for(int i = 0; i < InpMaxPositionsPerSide; i++)
   {
   ...
   Print("Grid initialized: ", InpMaxPositionsPerSide, " levels each side");
}

//+------------------------------------------------------------------+
//| Update grid occupancy from current positions - CRITICAL!         |
//+------------------------------------------------------------------+
void UpdateGridOccupancy()
{
   ...
   double gap = refPrice * InpGapPercent / 100.0;
   double levelTolerance = gap * 0.25; // 25% for matching
...
//+------------------------------------------------------------------+
//| Check grid levels STRICT - ONE POSITION PER LEVEL ONLY           |
//+------------------------------------------------------------------+
void CheckGridLevelsStrict()
{
...
//+------------------------------------------------------------------+
//| Check if position exists at price - double verification          |
//+------------------------------------------------------------------+
...
bool OpenGridPositionRobust(ENUM_ORDER_TYPE type, double gridLevel, int levelIndex)
{
   ...
   if(result)
   {
      ...
      Print("SUCCESS: ", EnumToString(type), " L", levelIndex+1, " @", DoubleToString(gridLevel, symbolDigits), 
...
//+------------------------------------------------------------------+
//| Handle trade errors appropriately                                |
//+------------------------------------------------------------------+
...
void ResetGridAfterClose()
{
   refPrice = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   InitializeGridLevels();
   Print("Grid reset: Ref=", DoubleToString(refPrice, symbolDigits));
}
...
void CreatePanel()
{
   ...
   CreateLabel("ToramaPanelTitle", "TORAMA GRID PRO ROBUST", panelX + 10, panelY + 8, 10, clrWhiteSmoke, ANCHOR_LEFT_UPPER);
