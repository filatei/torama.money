//+------------------------------------------------------------------+
//|                  TORAMA_MomentumGrid_IndividualTP_v3.11.mq5      |
//|                                          TORAMA CAPITAL           |
//+------------------------------------------------------------------+
#property copyright "TORAMA CAPITAL"
#property link      "https://toramacapital.com"
#property version   "3.11"
#property description "Momentum Grid EA - Simplified & Sacrosanct"
#property description "✓ No Auto-Reversal | ✓ Sacrosanct Reference (Default)"
...
input double InpGridGapPercent = 0.1;
...
input double InpTPOffsetPercent = 90.0;           // TP Offset % of Grid Gap (95% = just before next level)
...
input bool   InpForceMarketOrders = true;        // Force Market Orders (All Symbols)
input bool   InpAutoResetGrid = false;            // Auto-Reset Grid (Global TP/Close All)
...
int OnInit()
{
   ...
   PrintFormat("╔════════════════════════════════════════════════════════════════╗");
   PrintFormat("║  TORAMA Grid v3.11 COMPLETE - Magic: %I64d", magicNumber);
   PrintFormat("║  ✓ Sacrosanct Reference (Default) | ✓ Individual TP: %s", 
               InpEnableIndividualTP ? "ON" : "OFF");
   PrintFormat("║  ✓ Position Preservation: %d existing positions maintained", currentPositions);
   PrintFormat("║  Reference: ONLY resets via 'RESET REFERENCE' button");
   PrintFormat("║  Auto-Reset: %s | Lot: %.2f | Gap: %.2f%%", 
               InpAutoResetGrid ? "ON" : "OFF (Sacrosanct)", 
...
void OnTick()
{
   ...
   if(CheckGlobalTakeProfit())
   {
      ...
      {
         ResetSacrosanctGrid();
         PrintFormat("✓ Grid auto-reset: New reference at current price");
      }
      else
      {
         PrintFormat("⚠ Grid preserved: Reference price unchanged (Auto-Reset: OFF)");
         // Rebuild grid from existing reference without resetting levels
   ...
   if(GetTotalPositions() == 0 && gridInitialized && !isStoppedByDrawdown && (buyTriggeredCount > 0 || sellTriggeredCount > 0))
   {
      ...
      {
         ResetSacrosanctGrid();
         PrintFormat("✓ All positions closed externally - Grid auto-reset");
      }
      else
      {
         PrintFormat("⚠ All positions closed externally - Grid preserved (Auto-Reset: OFF)");
         // Clear triggered levels but keep reference price
         ArrayInitialize(triggeredBuyLevels, -999999);
         ArrayInitialize(triggeredSellLevels, -999999);
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam)
{
   ...
   if(sparam == panelPrefix + "BtnCloseAll")
   {
      ...
      {
         ResetSacrosanctGrid();
         PrintFormat("✓ Grid auto-reset after Close All");
      }
      else
      {
         PrintFormat("⚠ Grid preserved after Close All (Auto-Reset: OFF)");
         // Clear triggered levels but keep reference price
         ArrayInitialize(triggeredBuyLevels, -999999);
         ArrayInitialize(triggeredSellLevels, -999999);
         buyTriggeredCount = sellTriggeredCount = 0;
   ...
   else if(sparam == panelPrefix + "BtnTakeProfit")
   {
      ...
      // Take TP does NOT reset reference - respects Auto-Reset setting
      if(InpAutoResetGrid)
      {
         ResetSacrosanctGrid();
         PrintFormat("✓ Grid auto-reset after Take TP");
      }
      else
      {
         PrintFormat("⚠ Grid preserved after Take TP (Auto-Reset: OFF)");
         // Clear triggered levels but keep reference price
         ArrayInitialize(triggeredBuyLevels, -999999);
         ArrayInitialize(triggeredSellLevels, -999999);
         buyTriggeredCount = sellTriggeredCount = 0;
   ...
   {
      PrintFormat("╔════════════════════════════════════════════════════════════════╗");
      PrintFormat("║  MANUAL REFERENCE RESET");
      PrintFormat("║  Old Reference: %s", DoubleToString(referencePrice, dgt));
      
      // ALWAYS reset reference on this button - this is the ONLY manual reference reset
      ResetGridReference();
      
      PrintFormat("║  New Reference: %s", DoubleToString(referencePrice, dgt));
      PrintFormat("║  This is the ONLY way to manually reset grid reference");
      PrintFormat("╚════════════════════════════════════════════════════════════════╝");
   }
...
void UpdatePanelFast()
{
   ...
   ObjSetS("Spr", Fmt(currentSpread / pt, 1));
   ObjSetS("Ref", Fmt(referencePrice, 0));
   ObjSetS("Eq", "$" + Fmt(equity, 2));
   ObjSetS("Pk", "$" + Fmt(peakEquity, 2));
   
   // Show gap as ($XXX) 0.XX%
   ObjSetS("Gg", StringFormat("($%s) %.2f%%", Fmt(gridGapPrice, 2), InpGridGapPercent));
   ...
   ObjSetS("Nb", Fmt(nextBuyPrice, 2));
   ObjSetS("Ns", Fmt(nextSellPrice, 2));
   ...
   if(InpDailyProfitTargetPercent > 0)
   {
      ...
      ObjSetS("Daily", StringFormat("$%s (%.2f%%)", Fmt(dailyProfit, 2), dailyProfitPercent));
...
void CreatePanel()
{
   ...
   // Solid background - on top of everything
   CreateRect("BG", x, y, w, 450, InpPanelColor, false); y += 10;
   
   CreateRect("Hdr", x + 10, y, w - 20, 32, InpHeaderColor, false);
   CreateTxt("Title", x + 28, y + 8, "🔶 TORAMA GRID v3.11", clrWhite, 11, "Arial Black");
   CreateTxt("IndTP", x + w - 70, y + 10, InpEnableIndividualTP ? "IndTP✓" : "", clrLimeGreen, 9, "Arial Bold"); 
   y += 38;
   
   int x1 = x + 20, x2 = x + 200;
   ...
   CreateTxt("Dir", x2, y, dirText, clrWhite, 9, "Arial Bold"); y += lh;
   
   // Row 2: Symbol | Magic
   CreateTxt("SyL", x1, y, "Symbol:", C'120,120,120', 9);
   CreateTxt("Sym", x1 + 60, y, sym, clrWhite, 9, "Arial Bold");
   CreateTxt("MgL", x2, y, "Magic:", C'120,120,120', 9);
   CreateTxt("Mag", x2 + 50, y, IntegerToString(magicNumber), clrWhite, 8); y += lh;
   
   // Row 3: Lot | Spread | Ref
   CreateTxt("LoL", x1, y, "Lot:", C'120,120,120', 9);
   CreateTxt("Lot", x1 + 35, y, StringFormat("%.2f", effectiveInitialLotSize), clrWhite, 8, "Arial Bold");
   CreateTxt("SpL", x1 + 105, y, "Spr:", C'120,120,120', 9);
   CreateTxt("Spr", x1 + 140, y, "0.0", clrWhite, 8, "Arial Bold");
   CreateTxt("ReL", x2 + 20, y, "Ref:", C'120,120,120', 9);
   CreateTxt("Ref", x2 + 55, y, "0", clrWhite, 8, "Arial Bold"); y += lh;
   
   // Row 4: Equity | Peak | Gap ($XXX) 0.XX%
   CreateTxt("EqL", x1, y, "Eq:", C'120,120,120', 9);
   CreateTxt("Eq", x1 + 35, y, "$0", C'240,248,255', 8, "Arial Bold");
   CreateTxt("PkL", x1 + 120, y, "Pk:", C'120,120,120', 9);
   CreateTxt("Pk", x1 + 150, y, "$0", clrGold, 8, "Arial Bold");
   CreateTxt("GgL", x2 + 20, y, "Gap:", C'120,120,120', 9);
   CreateTxt("Gg", x2 + 55, y, "($0) 0.00%", clrWhite, 8, "Arial Bold"); y += lh;
   
   // Row 5: Trig | Cyc
   CreateTxt("TrL", x1, y, "Trig:", clrOrange, 9, "Arial Bold");
   CreateTxt("Tr", x1 + 45, y, "B:0|S:0", clrOrange, 8, "Arial Bold");
   CreateTxt("CyL", x2 - 10, y, "Cyc:", clrLimeGreen, 9, "Arial Bold");
   CreateTxt("Cyc", x2 + 35, y, "TP:0|CA:0|T:0", clrLimeGreen, 8, "Arial Bold"); y += lh;
   ...
   CreateTxt("Mo", x1 + 50, y, modeText, modeColor, 9, "Arial Bold"); 
   CreateTxt("DbgMode", x2, y, "", clrYellow, 9, "Arial Bold"); y += lh;
   
   CreateRect("S2", x + 10, y, w - 20, 1, clrDimGray, false); y += 7;
   
   // Row 7: Next Buy | Next Sell
   CreateTxt("NbL", x1, y, "Next Buy:", clrDodgerBlue, 9, "Arial Bold");
   CreateTxt("Nb", x1 + 75, y, "---", clrDodgerBlue, 9, "Arial Bold");
   CreateTxt("NsL", x2, y, "Next Sell:", clrTomato, 9, "Arial Bold");
   CreateTxt("Ns", x2 + 75, y, "---", clrTomato, 9, "Arial Bold"); y += lh;
   
   // Row 8: Buy Lots | Sell Lots
   CreateTxt("BlL", x1, y, "Buy Lots:", clrDodgerBlue, 9, "Arial Bold");
   CreateTxt("Bl", x1 + 75, y, "0.00", clrDodgerBlue, 9, "Arial Bold");
   CreateTxt("SlL", x2, y, "Sell Lots:", clrTomato, 9, "Arial Bold");
   CreateTxt("Sl", x2 + 75, y, "0.00", clrTomato, 9, "Arial Bold"); y += lh;
   
   // Row 9: Buy Levels | Sell Levels
   CreateTxt("BgL", x1, y, "Buy:", clrDodgerBlue, 9, "Arial Bold");
   CreateTxt("Bg", x1 + 40, y, "0 lvls", clrDodgerBlue, 9, "Arial Bold");
   CreateTxt("SgL", x2, y, "Sell:", clrTomato, 9, "Arial Bold");
   CreateTxt("Sg", x2 + 40, y, "0 lvls", clrTomato, 9, "Arial Bold"); y += lh;
   
   CreateRect("S3", x + 10, y, w - 20, 1, clrDimGray, false); y += 7;
   
   // Row 10: Profit | Target
   CreateTxt("PrL", x1, y, "Profit:", C'120,120,120', 9);
   CreateTxt("Pr", x1 + 55, y, "$0", clrLimeGreen, 10, "Arial Black");
   CreateTxt("TpL", x2, y, "Target:", C'120,120,120', 9);
   CreateTxt("Tp", x2 + 55, y, "$" + Fmt(InpGlobalTakeProfitUSD, 0), clrGold, 9, "Arial Bold"); y += lh;
   
   // Row 11: DD | Max DD
   CreateTxt("DdL", x1, y, "DD:", C'120,120,120', 9);
   CreateTxt("Dd", x1 + 55, y, "0.00%", clrWhite, 9, "Arial Bold");
   CreateTxt("MdL", x2, y, "Max DD:", C'120,120,120', 9);
   CreateTxt("Md", x2 + 55, y, StringFormat("%.1f%%", InpMaxDrawdownPercent), clrOrangeRed, 9, "Arial Bold"); y += lh;
   
   // Row 12: Daily Profit (if enabled)
   if(InpDailyProfitTargetPercent > 0)
   {
      CreateTxt("DailyL", x1, y, "Daily:", C'120,120,120', 9);
      CreateTxt("Daily", x1 + 55, y, "$0 (0.00%)", clrLimeGreen, 9, "Arial Bold");
      CreateTxt("DailyTgtL", x2, y, "Tgt:", C'120,120,120', 9);
      CreateTxt("DailyTgt", x2 + 55, y, StringFormat("%.1f%%", InpDailyProfitTargetPercent), clrGold, 9, "Arial Bold"); y += lh;
   }
   
   y += 8;
   CreateRect("S4", x + 10, y, w - 20, 1, clrDimGray, false); y += 10;
   
   // Buttons - wider spacing
   CreateBtn("BtnCloseAll", x + 15, y, 110, 28, "CLOSE ALL", clrWhite, clrCrimson);
   CreateBtn("BtnPause", x + 135, y, 110, 28, "PAUSE", clrWhite, C'255,152,0');
   CreateBtn("BtnTakeProfit", x + 255, y, 110, 28, "TAKE TP", clrWhite, C'34,139,34'); y += 34;
   CreateBtn("BtnResetRef", x + 15, y, 350, 28, "RESET REFERENCE", clrWhite, C'65,105,225'); y += 36;
   
   // Branding at bottom right
   CreateTxt("Brand", x + w - 150, y, "TORAMA CAPITAL", clrGold, 9, "Arial Black");
   CreateTxt("Debug", x + 15, y, "Press 'D' for Debug", C'80,80,80', 7);
   ChartRedraw();
}
