...
#property description "Advanced Momentum Trail Grid with Price Action Micro-Reversals"
#property description "Features: 3-Stage Adaptive Trailing, Micro-Grid Diversification"
...
input double   RiskRewardRatio = 2.0;               // Risk:Reward Ratio (Not Used - Trailing Only)
...
input int      MinPointsBetweenEntries = 10;        // Minimum Points Between Entries

input group "===== ENTRY SYSTEM (Price Action) ====="
input int      ReversalLookback = 5;                // Reversal Detection Bars
input double   ReversalThreshold = 0.6;             // Reversal Strength (0.1-1.0)
input int      VolatilityPeriod = 20;               // Volatility Calculation Period
input double   MinVolatilityFilter = 0.0001;        // Minimum Volatility to Trade
input bool     UseTickVolumeFilter = true;          // Use Tick Volume Confirmation
...
input string   TradeComment = "TORAMA_MTG";         // Trade Comment
...
int OnInit() {
    ...
    Print("═════════════════════════════════════════════════════════");
    Print("    TORAMA Momentum Trail Grid EA Initialized");
    Print("    Symbol: ", _Symbol);
    Print("    Daily Risk Limit: ", DailyRiskPercent, "%");
    Print("    Grid Spacing: ", GridSpacing, " points");
...
void OnDeinit(const int reason) {
    ...
    Print("TORAMA Momentum Trail Grid EA stopped. Reason: ", reason);
}
...
void OnTick() {
    ...
    // Update current volatility every tick
...
void InitializeGridLevels() {
    ...
    // Initialize BUY grid levels (below current price)
    ...
    // Initialize SELL grid levels (above current price)
    for(int i = 0; i < MaxGridLevels; i++) {
        ...
        if(position.SelectByIndex(i)) {
            if(position.Symbol() == _Symbol && position.Magic() == MagicNumber) {
                ...
                for(int g = 0; g < MaxGridLevels; g++) {
                    if(MathAbs(posPrice - buyGridLevels[g].price) < gridSpacingPrice / 2) {
                ...
                for(int g = 0; g < MaxGridLevels; g++) {
                    if(MathAbs(posPrice - sellGridLevels[g].price) < gridSpacingPrice / 2) {
                        if(posType == POSITION_TYPE_BUY) {
...
//+------------------------------------------------------------------+
//| Check Entry Signals (Price Action Micro-Reversals)              |
//+------------------------------------------------------------------+
void CheckEntrySignals() {
    ...
    if(currentVolatility < MinVolatilityFilter) {
        return;
    }
    
    // Detect micro-reversals
    bool bullishReversal = DetectBullishReversal();
    bool bearishReversal = DetectBearishReversal();
    
    // BUY Signal
    if(bullishReversal) {
        // Find nearest grid level below current price
        for(int i = 0; i < MaxGridLevels; i++) {
            if(buyGridLevels[i].price < bid && !buyGridLevels[i].hasBuyPosition) {
                if(TimeCurrent() - buyGridLevels[i].lastEntryTime > 60) { // 1 min cooldown
                    ExecuteBuyOrder(buyGridLevels[i].price);
    ...
    // SELL Signal
    if(bearishReversal && BidirectionalGrid) {
        // Find nearest grid level above current price
        for(int i = 0; i < MaxGridLevels; i++) {
            if(sellGridLevels[i].price > ask && !sellGridLevels[i].hasSellPosition) {
                if(TimeCurrent() - sellGridLevels[i].lastEntryTime > 60) { // 1 min cooldown
                    ExecuteSellOrder(sellGridLevels[i].price);
...
bool DetectBullishReversal() {
    ...
    if(CopyHigh(_Symbol, PERIOD_CURRENT, 0, ReversalLookback + 1, highs) <= 0) return false;
    if(CopyLow(_Symbol, PERIOD_CURRENT, 0, ReversalLookback + 1, lows) <= 0) return false;
    if(CopyClose(_Symbol, PERIOD_CURRENT, 0, ReversalLookback + 1, closes) <= 0) return false;
    if(UseTickVolumeFilter) {
        if(CopyTickVolume(_Symbol, PERIOD_CURRENT, 0, ReversalLookback + 1, volumes) <= 0) return false;
    }
    
    // Check for price rejection at lows
    double currentLow = lows[0];
    double previousLow = lows[1];
    double currentClose = closes[0];
    
    // Wick ratio - lower wick should be significant
    double bodySize = MathAbs(closes[0] - iOpen(_Symbol, PERIOD_CURRENT, 0));
    double lowerWick = MathAbs(currentClose - currentLow);
    
    if(bodySize == 0) bodySize = pointValue;
    
    double wickRatio = lowerWick / bodySize;
    
    // Reversal conditions
    bool priceRejection = (currentLow < previousLow && currentClose > previousLow);
    bool strongWick = (wickRatio > ReversalThreshold);
    bool volumeConfirm = true;
    
    if(UseTickVolumeFilter && ArraySize(volumes) > 1) {
        volumeConfirm = (volumes[0] > volumes[1]);
    }
    
    return (priceRejection && strongWick && volumeConfirm);
}

//+------------------------------------------------------------------+
//| Detect Bearish Reversal                                          |
//+------------------------------------------------------------------+
bool DetectBearishReversal() {
    double highs[], lows[], closes[];
    long volumes[];
    ...
    // Check for price rejection at highs
    double currentHigh = highs[0];
    double previousHigh = highs[1];
    double currentClose = closes[0];
    
    // Wick ratio - upper wick should be significant
    double bodySize = MathAbs(closes[0] - iOpen(_Symbol, PERIOD_CURRENT, 0));
    double upperWick = MathAbs(currentHigh - currentClose);
    
    if(bodySize == 0) bodySize = pointValue;
    
    double wickRatio = upperWick / bodySize;
    
    // Reversal conditions
    bool priceRejection = (currentHigh > previousHigh && currentClose < previousHigh);
    bool strongWick = (wickRatio > ReversalThreshold);
    bool volumeConfirm = true;
...
//| Execute Buy Order                                                 |
//+------------------------------------------------------------------+
void ExecuteBuyOrder(double gridPrice) {
    double ask = symbolInfo.Ask();
    ...
    if(trade.Buy(lotSize, _Symbol, ask, sl, 0, TradeComment)) {
        ulong ticket = trade.ResultOrder();
        Print("BUY Order Opened: Ticket=", ticket, " Lots=", lotSize, " Price=", ask, " SL=", sl);
...
//| Execute Sell Order                                                |
//+------------------------------------------------------------------+
void ExecuteSellOrder(double gridPrice) {
    double bid = symbolInfo.Bid();
    double lotSize = CalculateLotSize();
    ...
    if(trade.Sell(lotSize, _Symbol, bid, sl, 0, TradeComment)) {
        ulong ticket = trade.ResultOrder();
        Print("SELL Order Opened: Ticket=", ticket, " Lots=", lotSize, " Price=", bid, " SL=", sl);
...
double CalculateLotSize() {
    ...
    if(UseVolatilityCircuit && baseVolatility > 0) {
        if(currentVolatility > baseVolatility * VolatilitySpikeFactor) {
            riskPerPosition *= 0.5; // Reduce position size by 50%
        }
    }
        ...
        if(position.SelectByIndex(i)) {
            if(position.Symbol() == _Symbol && position.Magic() == MagicNumber) {
                ...
                if(trackerIndex >= 0) {
                    ...
                    // Determine trail stage and calculate new SL
...
double CalculateTrailingStop(ulong ticket, int trackerIndex, double profitPoints) {
    ...
    // STAGE 1: Rapid Breakeven (0 - Stage1_BE_Points)
    ...
    // STAGE 3: Momentum Trail (Stage2_MaxPoints+)
        ...
        // Consider correlation if enabled
        if(UseCorrelationTrailing) {
            int profitablePositions = CountProfitablePositions();
            if(profitablePositions > 3) {
                trailDistance *= 1.2; // Tighten trail when many positions profitable
            } else if(profitablePositions == 1) {
                trailDistance *= 0.8; // Give more room when only one profitable
            }
        }
...
//| Calculate Base Volatility                                         |
//+------------------------------------------------------------------+
void CalculateBaseVolatility() {
    double prices[];
    ArraySetAsSeries(prices, true);
    
    if(CopyClose(_Symbol, PERIOD_CURRENT, 0, VolatilityPeriod, prices) > 0) {
        double sum = 0;
        double mean = 0;
        ...
        baseVolatility = MathSqrt(sum / VolatilityPeriod);
    }
}
...
//+------------------------------------------------------------------+
void CalculateCurrentVolatility() {
    double prices[];
    ArraySetAsSeries(prices, true);
    
    int lookback = MathMin(VolatilityPeriod, 10); // Short-term volatility
    
    if(CopyClose(_Symbol, PERIOD_CURRENT, 0, lookback, prices) > 0) {
        double sum = 0;
        double mean = 0;
        ...
        currentVolatility = MathSqrt(sum / lookback);
    }
}
...
void CreateInfoPanel() {
    ...
    ObjectSetInteger(0, prefix + "PanelBG", OBJPROP_YSIZE, 280);
    ...
    ObjectSetString(0, prefix + "Header", OBJPROP_TEXT, "TORAMA MOMENTUM TRAIL GRID");
...
void UpdateInfoPanel() {
    ...
    string values[] = {
        ...
        DoubleToString(currentVolatility / pointValue, 1) + " pts",
