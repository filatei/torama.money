...
void CreatePanel()
{
   ...
   CreateText(panelPrefix + "GlobalTP", xRight + 50, yOffset, StringFormat("$%.0f", InpGlobalTakeProfitUSD), clrGold, 9, "Arial Bold");
   yOffset += lineHeight + 2;
...
void UpdatePanel()
{
   ...
   ObjectSetString(0, panelPrefix + "Equity", OBJPROP_TEXT, StringFormat("$%.2f", equity));
   
   //--- Reference Price (SACROSANCT)
   ObjectSetString(0, panelPrefix + "RefPrice", OBJPROP_TEXT, StringFormat("%.*f", dgt, referencePrice));
   ...
   ObjectSetString(0, panelPrefix + "GridGap", OBJPROP_TEXT, 
                  StringFormat("%.2f%% (%.*f)", InpGridGapPercent, dgt, gridGapPrice));
   
   //--- Next Buy/Sell (from pending orders)
   if(nextBuy > 0)
      ObjectSetString(0, panelPrefix + "NextBuy", OBJPROP_TEXT, StringFormat("%.*f", dgt, nextBuy));
   else
      ObjectSetString(0, panelPrefix + "NextBuy", OBJPROP_TEXT, "---");
   
   if(nextSell > 0)
      ObjectSetString(0, panelPrefix + "NextSell", OBJPROP_TEXT, StringFormat("%.*f", dgt, nextSell));
   else
