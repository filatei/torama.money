//+------------------------------------------------------------------+
//|                    TORAMA Universal Grid EA v3.14                |
//|                                           TORAMA CAPITAL          |
...
input bool     ReturnToNeutralOnAllClose = true; // Return to NEUTRAL when all positions close
...
void OnTick()
{
   ...
   if(lastPositionCount > 0 && currentPositionCount == 0)
   {
      ...
      if(ReturnToNeutralOnAllClose && currentMode != MODE_NEUTRAL)
      {
         ...
         Print("⚪ RETURNED TO NEUTRAL: All positions closed in ", previousMode, " mode");
         Print("   New reference: $", DoubleToString(referencePrice, 2));
         Print("   Waiting for price to move ±1 grid level ($", DoubleToString(currentGapSize, 2), ")");
      }
   }
