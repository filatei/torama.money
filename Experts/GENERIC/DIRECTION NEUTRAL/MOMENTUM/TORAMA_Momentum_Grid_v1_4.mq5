...
#property description "Momentum Grid EA - FIXED TP/SL calculation"
...
int OnInit()
{
   ...
   // Print startup info
   Print("╔═══════════════════════════════════════════╗");
   Print("║  TORAMA MOMENTUM GRID v1.2               ║");
   Print("╚═══════════════════════════════════════════╝");
...
void OnTick()
{
   ...
   if(buyCount == 0 && lastBuyCount > 0) needsRebuild = true;  // BUY side cleared
   if(sellCount == 0 && lastSellCount > 0) needsRebuild = true; // SELL side cleared
...
bool OpenPosition(ENUM_ORDER_TYPE type, double price)
{
   ...
   if(IndividualTPDollars > 0) {
      // Get contract size (100 oz for gold, varies by symbol)
      double contractSize = SymbolInfoDouble(_Symbol, SYMBOL_TRADE_CONTRACT_SIZE);
      
      // Calculate price distance needed for target profit
      // Formula: profit = price_distance × (lot_size × contract_size)
      // Therefore: price_distance = profit / (lot_size × contract_size)
...
void CreatePanel()
{
   ...
   CreateLbl("H", "═══ TORAMA MOMENTUM GRID ═══", x + 10, cy, hdr, 9, "Arial Bold"); cy += 23;
