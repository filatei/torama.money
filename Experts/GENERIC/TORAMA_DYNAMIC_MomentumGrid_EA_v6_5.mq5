//+------------------------------------------------------------------+
//|                        TORAMA Dynamic Momentum Grid EA           |
//|                                           TORAMA CAPITAL          |
...
#property description "PROFESSIONAL UI: Larger fonts, Segoe UI, bold H4 levels"
#property description "v6.3: BOTH mode - positions coexist on reversals"
#property description "v6.4: Volatility-based grid adjustment + Max DD stop fix"
#property description "v6.5: D1 Support/Resistance levels (R1-R3, S1-S3)"

//--- EA Version (single source of truth)
#define EA_VERSION "6.5"
#define EA_NAME "MOMENTUM GRID"

//--- Input Parameters
// input group "=== GRID SETTINGS ==="
input double   GridSpacingPercent = 0.01;        // Grid spacing % (0.3-0.5 recommended)
input int      ReversalGaps = 4;                // Gaps to trigger reversal (3-4 recommended)
input int      MaxPositions = 50;               // Maximum positions per direction
input int      MaxSpread = 200;                 // Maximum spread in points

// input group "=== VOLATILITY ADJUSTMENT ==="
input bool     EnableVolatilityAdjustment = true;  // Enable dynamic grid spacing based on volatility
input int      VolatilityATRPeriod = 14;           // ATR period for volatility measurement
input ENUM_TIMEFRAMES VolatilityTimeframe = PERIOD_M15;  // Timeframe for ATR calculation
input double   VolatilityMultiplierLow = 1.0;      // Grid multiplier when volatility is LOW (1.0 = normal)
input double   VolatilityMultiplierHigh = 2.5;     // Grid multiplier when volatility is HIGH (2.5 = wider gaps)
input double   VolatilityThresholdLow = 50.0;      // ATR below this = LOW volatility (Gold: 50, BTC: 100)
input double   VolatilityThresholdHigh = 150.0;    // ATR above this = HIGH volatility (Gold: 150, BTC: 300)
...
input double   IndividualTPDollars = 25.0;      // TP per position in dollars (SACROSANCT)
...
input double   GlobalTPDollars = 200.0;          // Global profit target in dollars (optional)
...
SRLevel supportLevels[];        // H4 support levels
SRLevel resistanceLevels[];     // H4 resistance levels
SRLevel d1SupportLevels[];      // D1 support levels
SRLevel d1ResistanceLevels[];   // D1 resistance levels
...
// Volatility adjustment tracking
double currentVolatilityATR = 0;
double volatilityMultiplier = 1.0;
datetime lastVolatilityCheck = 0;
...
// D1 S/R tracking
datetime lastD1Update = 0;
bool showD1Levels = true;  // Always show D1 levels on chart
...
int OnInit()
{
   ...
   volatilityStatus = EnableVolatilityAdjustment ? "NORMAL" : "DISABLED";
   lastVolatilityCheck = 0;
   currentVolatilityATR = 0;
   
   if(EnableVolatilityAdjustment)
   {
      Print("--- VOLATILITY ADJUSTMENT ---");
      Print("Volatility Adjustment: ENABLED");
      Print("ATR Period: ", VolatilityATRPeriod);
      Print("Timeframe: ", EnumToString(VolatilityTimeframe));
      Print("Low Volatility (<", VolatilityThresholdLow, "): ", VolatilityMultiplierLow, "x grid");
      Print("High Volatility (>", VolatilityThresholdHigh, "): ", VolatilityMultiplierHigh, "x grid");
   }
   else
   {
      Print("--- VOLATILITY ADJUSTMENT ---");
      Print("Volatility Adjustment: DISABLED (static grid spacing)");
   }
   ...
   // ALWAYS calculate D1 S/R and draw on chart
   CalculateD1SupportResistance();
   DrawD1Levels();
   ...
   Print("--- D1 SUPPORT/RESISTANCE LEVELS ---");
   Print("D1 Levels: ENABLED (Always shown on chart)");
   Print("D1 Resistance Levels: ", ArraySize(d1ResistanceLevels), " (R1-R3)");
   Print("D1 Support Levels: ", ArraySize(d1SupportLevels), " (S1-S3)");
   if(ArraySize(d1ResistanceLevels) > 0)
   {
      int digits = (int)SymbolInfoInteger(_Symbol, SYMBOL_DIGITS);
      for(int i = 0; i < ArraySize(d1ResistanceLevels); i++)
      {
         Print("  R", (i+1), ": ", DoubleToString(d1ResistanceLevels[i].price, digits));
      }
   }
   if(ArraySize(d1SupportLevels) > 0)
   {
      int digits = (int)SymbolInfoInteger(_Symbol, SYMBOL_DIGITS);
      for(int i = 0; i < ArraySize(d1SupportLevels); i++)
      {
         Print("  S", (i+1), ": ", DoubleToString(d1SupportLevels[i].price, digits));
      }
   }
...
void OnDeinit(const int reason)
{
   ...
   ObjectsDeleteAll(0, "D1Level_");
   ObjectsDeleteAll(0, "D1Label_");
}
...
void OnTick()
{
   ...
   // Update D1 S/R levels daily
   MqlDateTime current_time;
   TimeToStruct(TimeCurrent(), current_time);
   
   // Update at midnight (start of new D1 candle)
   if(current_time.hour == 0 && current_time.min == 0)
   {
      if(lastD1Update != TimeCurrent())
      {
         CalculateD1SupportResistance();
         DrawD1Levels();
         lastD1Update = TimeCurrent();
      }
   }
   ...
   // Update volatility adjustment periodically
   UpdateVolatilityAdjustment();
   
   // Calculate grid spacing with volatility multiplier
   gridSpacing = currentPrice * GridSpacingPercent * volatilityMultiplier / 100.0;
   ...
   if(ArraySize(positions) > 0)
   {
      ...
      {
         Print("⛔ MAX DRAWDOWN LIMIT HIT: ", DoubleToString(MaxDrawdownPercent, 1), "%");
         Print("⛔ EA STOPPED - Close all positions and halt trading");
         
         // Close all positions first
         CloseAllPositions("Max Drawdown Protection");
         ...
         Alert("⛔ MOMENTUM EA STOPPED: Max Drawdown ", DoubleToString(MaxDrawdownPercent, 1), "% exceeded!");
   ...
   else
   {
      if(currentTrend == TREND_BUYING)
      {
         ...
         if(ShouldReverse(currentPrice))
         {
            ...
            {
               // TRADE_BOTH MODE: Close BUY positions, switch to SELLING
               // This is NORMAL behavior for news trading!
               // Reset state flag when actually reversing
               inReversalIgnoredState = false;
               
               Print("🔄 MOMENTUM REVERSAL: Price dropped ", ReversalGaps, " gaps - switching to SELLING");
               
               // Close all BUY positions first
               if(ArraySize(positions) > 0)
               {
                  CloseAllPositions("Reversal from BUY to SELL");
               }
               
               // Switch trend and open first SELL position
               currentTrend = TREND_SELLING;
            ...
            {
               // Would reverse to SELL but BUY ONLY mode (non-bidirectional) - close and wait
               Print("🔄 MOMENTUM REVERSAL SIGNAL but BUY ONLY mode - closing positions");
               CloseAllPositions("Reversal Signal - BUY ONLY Mode");
               currentTrend = TREND_WAITING;
               lastTrendPrice = currentPrice;
            }
         }
      ...
      else if(currentTrend == TREND_SELLING)
      {
         ...
         if(ShouldReverse(currentPrice))
         {
            ...
            {
               // TRADE_BOTH MODE: Close SELL positions, switch to BUYING
               // This is NORMAL behavior for news trading!
               // Reset state flag when actually reversing
               inReversalIgnoredState = false;
               
               Print("🔄 MOMENTUM REVERSAL: Price rose ", ReversalGaps, " gaps - switching to BUYING");
               
               // Close all SELL positions first
               if(ArraySize(positions) > 0)
               {
                  CloseAllPositions("Reversal from SELL to BUY");
               }
               
               // Switch trend and open first BUY position
               currentTrend = TREND_BUYING;
            ...
            {
               // Would reverse to BUY but SELL ONLY mode (non-bidirectional) - close and wait
               Print("🔄 MOMENTUM REVERSAL SIGNAL but SELL ONLY mode - closing positions");
               CloseAllPositions("Reversal Signal - SELL ONLY Mode");
               currentTrend = TREND_WAITING;
               lastTrendPrice = currentPrice;
            }
         }
   ...
   // Always draw D1 levels (updated daily)
   if(showD1Levels && (ArraySize(d1SupportLevels) > 0 || ArraySize(d1ResistanceLevels) > 0))
      DrawD1Levels();
}
...
//+------------------------------------------------------------------+
//| Calculate D1 Support and Resistance levels (3 each: R1-R3, S1-S3)|
//+------------------------------------------------------------------+
void CalculateD1SupportResistance()
{
   ArrayResize(d1SupportLevels, 0);
   ArrayResize(d1ResistanceLevels, 0);
   
   double highs[], lows[];
   ArraySetAsSeries(highs, true);
   ArraySetAsSeries(lows, true);
   
   // Use 100 D1 bars for calculation (about 3-4 months)
   int d1LookbackBars = 100;
   
   int copied_high = CopyHigh(_Symbol, PERIOD_D1, 0, d1LookbackBars, highs);
   int copied_low = CopyLow(_Symbol, PERIOD_D1, 0, d1LookbackBars, lows);
   
   if(copied_high <= 0 || copied_low <= 0)
   {
      Print("⚠️ Failed to copy D1 price data");
      return;
   }
   
   // Find swing highs (resistance) - stronger condition for D1
   for(int i = 3; i < d1LookbackBars - 3; i++)
   {
      // D1 swing high: current high > previous 3 and next 3 highs
      if(highs[i] > highs[i-1] && highs[i] > highs[i-2] && highs[i] > highs[i-3] &&
         highs[i] > highs[i+1] && highs[i] > highs[i+2] && highs[i] > highs[i+3])
      {
         // Check if this level already exists
         bool exists = false;
         for(int j = 0; j < ArraySize(d1ResistanceLevels); j++)
         {
            if(MathAbs(d1ResistanceLevels[j].price - highs[i]) < (highs[i] * 0.002)) // Within 0.2%
            {
               exists = true;
               d1ResistanceLevels[j].touches++;
               break;
            }
         }
         
         if(!exists && ArraySize(d1ResistanceLevels) < 3)  // Only 3 resistance levels
         {
            int newSize = ArraySize(d1ResistanceLevels) + 1;
            ArrayResize(d1ResistanceLevels, newSize);
            d1ResistanceLevels[newSize-1].price = highs[i];
            d1ResistanceLevels[newSize-1].touches = 1;
            d1ResistanceLevels[newSize-1].isResistance = true;
         }
      }
   }
   
   // Find swing lows (support) - stronger condition for D1
   for(int i = 3; i < d1LookbackBars - 3; i++)
   {
      // D1 swing low: current low < previous 3 and next 3 lows
      if(lows[i] < lows[i-1] && lows[i] < lows[i-2] && lows[i] < lows[i-3] &&
         lows[i] < lows[i+1] && lows[i] < lows[i+2] && lows[i] < lows[i+3])
      {
         bool exists = false;
         for(int j = 0; j < ArraySize(d1SupportLevels); j++)
         {
            if(MathAbs(d1SupportLevels[j].price - lows[i]) < (lows[i] * 0.002))
            {
               exists = true;
               d1SupportLevels[j].touches++;
               break;
            }
         }
         
         if(!exists && ArraySize(d1SupportLevels) < 3)  // Only 3 support levels
         {
            int newSize = ArraySize(d1SupportLevels) + 1;
            ArrayResize(d1SupportLevels, newSize);
            d1SupportLevels[newSize-1].price = lows[i];
            d1SupportLevels[newSize-1].touches = 1;
            d1SupportLevels[newSize-1].isResistance = false;
         }
      }
   }
   ...
   // Sort by proximity to current price (most relevant first)
   double currentPrice = (SymbolInfoDouble(_Symbol, SYMBOL_ASK) + SymbolInfoDouble(_Symbol, SYMBOL_BID)) / 2;
   SortSRLevelsByProximity(d1SupportLevels, currentPrice);
   SortSRLevelsByProximity(d1ResistanceLevels, currentPrice);
   
   Print("📊 D1 S/R Updated: ", ArraySize(d1SupportLevels), " Support, ", 
         ArraySize(d1ResistanceLevels), " Resistance levels found");
}
...
//+------------------------------------------------------------------+
//| Draw D1 Support/Resistance levels on chart (horizontal lines)    |
//+------------------------------------------------------------------+
void DrawD1Levels()
{
   // Delete existing D1 levels
   ObjectsDeleteAll(0, "D1Level_");
   ObjectsDeleteAll(0, "D1Label_");
   
   int digits = (int)SymbolInfoInteger(_Symbol, SYMBOL_DIGITS);
   
   // Draw D1 resistance levels (R1, R2, R3)
   for(int i = 0; i < ArraySize(d1ResistanceLevels); i++)
   {
      string levelName = "D1Level_R" + IntegerToString(i+1);
      string labelName = "D1Label_R" + IntegerToString(i+1);
      
      // Create horizontal line for resistance
      ObjectCreate(0, levelName, OBJ_HLINE, 0, 0, d1ResistanceLevels[i].price);
      ObjectSetInteger(0, levelName, OBJPROP_COLOR, clrCrimson);
      ...
      // Create label for resistance
      ObjectCreate(0, labelName, OBJ_TEXT, 0, TimeCurrent(), d1ResistanceLevels[i].price);
      ObjectSetString(0, labelName, OBJPROP_TEXT, "D1 R" + IntegerToString(i+1) + " - " + DoubleToString(d1ResistanceLevels[i].price, digits));
      ObjectSetInteger(0, labelName, OBJPROP_COLOR, clrCrimson);
   ...
   // Draw D1 support levels (S1, S2, S3)
   for(int i = 0; i < ArraySize(d1SupportLevels); i++)
   {
      string levelName = "D1Level_S" + IntegerToString(i+1);
      string labelName = "D1Label_S" + IntegerToString(i+1);
      
      // Create horizontal line for support
      ObjectCreate(0, levelName, OBJ_HLINE, 0, 0, d1SupportLevels[i].price);
      ObjectSetInteger(0, levelName, OBJPROP_COLOR, clrLimeGreen);
      ObjectSetInteger(0, levelName, OBJPROP_STYLE, STYLE_SOLID);
      ObjectSetInteger(0, levelName, OBJPROP_WIDTH, 2);
      ObjectSetInteger(0, levelName, OBJPROP_BACK, false);
      ObjectSetInteger(0, levelName, OBJPROP_SELECTABLE, true);
      ...
      // Create label for support
      ObjectCreate(0, labelName, OBJ_TEXT, 0, TimeCurrent(), d1SupportLevels[i].price);
      ObjectSetString(0, labelName, OBJPROP_TEXT, "D1 S" + IntegerToString(i+1) + " - " + DoubleToString(d1SupportLevels[i].price, digits));
   ...
   if(ArraySize(d1ResistanceLevels) > 0 || ArraySize(d1SupportLevels) > 0)
   {
      Print("✅ D1 Levels Drawn: ", ArraySize(d1ResistanceLevels), " Resistance, ", 
            ArraySize(d1SupportLevels), " Support");
   }
}
...
//+------------------------------------------------------------------+
//| Update volatility-based grid multiplier                          |
//+------------------------------------------------------------------+
void UpdateVolatilityAdjustment()
{
   if(!EnableVolatilityAdjustment)
   {
      volatilityMultiplier = 1.0;
      volatilityStatus = "DISABLED";
      return;
   }
   
   // Check volatility every 5 minutes to avoid excessive calculations
   ...
   int handle = iATR(_Symbol, VolatilityTimeframe, VolatilityATRPeriod);
   if(handle == INVALID_HANDLE)
   {
      Print("❌ Failed to create Volatility ATR indicator handle");
      volatilityMultiplier = 1.0;
      return;
   }
   ...
   if(CopyBuffer(handle, 0, 0, 1, atr) <= 0)
   {
      Print("❌ Failed to copy Volatility ATR buffer");
      IndicatorRelease(handle);
      volatilityMultiplier = 1.0;
      return;
   }
   
   currentVolatilityATR = atr[0];
   IndicatorRelease(handle);
   
   // Determine volatility multiplier based on ATR thresholds
   double oldMultiplier = volatilityMultiplier;
   string oldStatus = volatilityStatus;
   
   if(currentVolatilityATR <= VolatilityThresholdLow)
   {
      // LOW volatility - use tight grid (normal or tighter)
      volatilityMultiplier = VolatilityMultiplierLow;
      volatilityStatus = "LOW";
   }
   else if(currentVolatilityATR >= VolatilityThresholdHigh)
   {
      // HIGH volatility - use wide grid
      volatilityMultiplier = VolatilityMultiplierHigh;
      volatilityStatus = "HIGH";
   }
   else
   {
      // NORMAL volatility - interpolate between low and high
      double range = VolatilityThresholdHigh - VolatilityThresholdLow;
      double position = (currentVolatilityATR - VolatilityThresholdLow) / range;
      volatilityMultiplier = VolatilityMultiplierLow + (position * (VolatilityMultiplierHigh - VolatilityMultiplierLow));
      volatilityStatus = "NORMAL";
   }
   
   // Log significant changes
   if(MathAbs(volatilityMultiplier - oldMultiplier) > 0.1 || volatilityStatus != oldStatus)
   {
      Print("═══════════════════════════════════════════════════════════════");
      Print("📊 VOLATILITY ADJUSTMENT UPDATE");
      Print("ATR (", EnumToString(VolatilityTimeframe), "): ", DoubleToString(currentVolatilityATR, 2));
      Print("Status: ", oldStatus, " → ", volatilityStatus);
      Print("Grid Multiplier: ", DoubleToString(oldMultiplier, 2), "x → ", DoubleToString(volatilityMultiplier, 2), "x");
      Print("Effective Grid: ", DoubleToString(GridSpacingPercent * volatilityMultiplier, 3), "%");
      Print("═══════════════════════════════════════════════════════════════");
   }
}
...
void AdjustParametersForRegime()
{
   ...
   // Recalculate grid spacing for current price with volatility multiplier
   double currentPrice = (SymbolInfoDouble(_Symbol, SYMBOL_ASK) + SymbolInfoDouble(_Symbol, SYMBOL_BID)) / 2;
   gridSpacing = currentPrice * workingGridPercent * volatilityMultiplier / 100.0;
   
   Print("Grid Spacing Recalculated: $", DoubleToString(gridSpacing, 2));
   if(EnableVolatilityAdjustment)
   {
      Print("Volatility Multiplier: ", DoubleToString(volatilityMultiplier, 2), "x (", volatilityStatus, ")");
   }
}
...
double CalculateGlobalPnL()
{
   ...
   for(int i = 0; i < ArraySize(positions); i++)
   {
      ...
      {
         totalPnL += (currentBid - positions[i].entryPrice) * LotSize * 100;
      }
      else
      {
         totalPnL += (positions[i].entryPrice - currentAsk) * LotSize * 100;
      }
   }
...
bool CheckDrawdownLimit()
{
   ...
   {
      Print("⚠️ DRAWDOWN LIMIT EXCEEDED: ", DoubleToString(drawdownPct, 2), "%");
      return true;
   }
...
void UpdateInfoPanel()
{
   ...
   if(EnableVolatilityAdjustment)
   {
      gridText = StringFormat("Grid: %.2f%% (x%.1f %s)", 
                              GridSpacingPercent, 
   ...
   {
      gridText = StringFormat("Grid: %.2f%%", GridSpacingPercent);
   }
