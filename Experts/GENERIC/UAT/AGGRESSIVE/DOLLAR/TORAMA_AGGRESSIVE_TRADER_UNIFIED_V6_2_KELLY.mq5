//+------------------------------------------------------------------+
//|              TORAMA Aggressive Trader UNIFIED v6.2 KELLY         |
//|                                           TORAMA CAPITAL          |
...
#property description "Unified BUY/SELL Grid - 20% KELLY CRITERION LOT SIZING"
#property description "Mathematically optimal lot sizing based on win rate & profit/loss ratio"
#property description "6X TP when opposite LOT exposure >= 80% - Self-adaptive performance"
#property description "Thousands separator - Next Buy/Sell levels - Professional risk management"

#define EA_VERSION "6.20"
#define EA_NAME "TORAMA AGGRESSIVE UNIFIED KELLY"
...
input group "=== KELLY CRITERION LOT SIZING ==="
input bool     EnableKellyLotSizing = true;           // Enable Kelly Criterion lot sizing
input double   KellyFraction = 0.20;                  // Kelly fraction (0.20 = 20% conservative)
input int      MinTradesForKelly = 20;                // Min trades before using Kelly (default until stats available)
...
// Kelly Criterion historical stats
...
HistoricalStats buyStats;
HistoricalStats sellStats;

// Track last known position counts to detect closes
int lastKnownBuyCount = 0;
int lastKnownSellCount = 0;
...
int OnInit()
{
   ...
   Print("   Kelly lot sizing enabled:", EnableKellyLotSizing ? "YES" : "NO");
   ...
   // Initialize Kelly Criterion stats with optimistic defaults
   buyStats.totalTrades = 0;
   buyStats.winningTrades = 0;
   buyStats.totalProfit = 0;
   buyStats.totalLoss = 0;
   buyStats.avgWin = IndividualTPDollars;  // Start with expected TP
   buyStats.avgLoss = IndividualTPDollars * 2.0;  // Conservative estimate
   buyStats.winRate = 0.85;  // Optimistic default until we have data
   
   sellStats.totalTrades = 0;
   sellStats.winningTrades = 0;
   sellStats.totalProfit = 0;
   sellStats.totalLoss = 0;
   sellStats.avgWin = IndividualTPDollars;
   sellStats.avgLoss = IndividualTPDollars * 2.0;
   sellStats.winRate = 0.85;
   
   if(EnableKellyLotSizing)
   {
      Print("📊 KELLY CRITERION ENABLED:");
      Print("   Kelly Fraction: ", DoubleToString(KellyFraction * 100, 0), "% (conservative)");
      Print("   Min trades for Kelly: ", MinTradesForKelly);
      Print("   Default win rate: 85% (until stats available)");
   }
...
//+------------------------------------------------------------------+
//| UPDATE HISTORICAL STATS FOR KELLY CRITERION                       |
//+------------------------------------------------------------------+
void UpdateKellyStats(string side, double profit)
{
   // Directly update the appropriate stats structure
   if(side == "BUY")
   {
      ...
      if(profit > 0)
      {
         buyStats.winningTrades++;
         buyStats.totalProfit += profit;
         
         if(buyStats.winningTrades > 0)
            buyStats.avgWin = buyStats.totalProfit / buyStats.winningTrades;
      }
      else
      {
         buyStats.totalLoss += MathAbs(profit);
         int losingTrades = buyStats.totalTrades - buyStats.winningTrades;
         
         if(losingTrades > 0)
            buyStats.avgLoss = buyStats.totalLoss / losingTrades;
      }
      
      // Update win rate
      if(buyStats.totalTrades > 0)
         buyStats.winRate = (double)buyStats.winningTrades / buyStats.totalTrades;
   }
   else  // SELL
   {
      ...
      if(profit > 0)
      {
         sellStats.winningTrades++;
         sellStats.totalProfit += profit;
         
         if(sellStats.winningTrades > 0)
            sellStats.avgWin = sellStats.totalProfit / sellStats.winningTrades;
      }
      else
      {
         sellStats.totalLoss += MathAbs(profit);
         int losingTrades = sellStats.totalTrades - sellStats.winningTrades;
         
         if(losingTrades > 0)
            sellStats.avgLoss = sellStats.totalLoss / losingTrades;
      }
      
      // Update win rate
      if(sellStats.totalTrades > 0)
         sellStats.winRate = (double)sellStats.winningTrades / sellStats.totalTrades;
   }
}

//+------------------------------------------------------------------+
//| GET WIN RATE FOR KELLY CALCULATION                               |
//+------------------------------------------------------------------+
double GetWinRate(string side)
{
   // Use default until we have enough trades
   if(side == "BUY")
   {
      if(buyStats.totalTrades < MinTradesForKelly)
         return 0.85;  // Optimistic default
      
      return buyStats.winRate;
   }
   else  // SELL
   {
      if(sellStats.totalTrades < MinTradesForKelly)
         return 0.85;  // Optimistic default
      
      return sellStats.winRate;
   }
}
...
double CalculateDynamicLot(string side)
{
   ...
   int ourCount = isBuySide ? ArraySize(BuySide.positions) : ArraySize(SellSide.positions);
   int oppositeCount = isBuySide ? ArraySize(SellSide.positions) : ArraySize(BuySide.positions);
   
   double baseLot = validatedLotSize;
   
   // If Kelly disabled or winning side maxed out, use base lot
   if(!EnableKellyLotSizing || ourCount > WinningSideMaxPositions)
   {
      return baseLot;
   }
   
   // Get historical stats for this side
   double winRate = GetWinRate(side);
   double avgWin = isBuySide ? buyStats.avgWin : sellStats.avgWin;
   double avgLoss = isBuySide ? buyStats.avgLoss : sellStats.avgLoss;
   avgLoss = MathMax(avgLoss, IndividualTPDollars);  // Prevent division by zero
   
   // Win/Loss ratio
   double R = avgWin / avgLoss;
   
   // Full Kelly formula: K = (W × R - L) / R
   double fullKelly = (winRate * R - (1.0 - winRate)) / R;
   
   // Apply Kelly fraction (20% conservative)
   double fractionalKelly = fullKelly * KellyFraction;
   
   // Adjust by opposite saturation (more saturation = more confidence)
   double saturation = (double)oppositeCount / MaxPositionsPerSide;
   double adjustedKelly = fractionalKelly * (0.5 + saturation);
   
   // Clamp Kelly to reasonable range (5% - 50%)
   adjustedKelly = MathMax(0.05, MathMin(adjustedKelly, 0.50));
   
   // Calculate lot size based on equity and Kelly percentage
   double equity = AccountInfoDouble(ACCOUNT_EQUITY);
   double equityRiskPerPosition = equity / 100.0;  // Risk 1% of equity per position max
   double kellyLot = (equityRiskPerPosition * adjustedKelly) / (avgLoss / baseLot);
   
   // Apply hard limits
   kellyLot = MathMax(baseLot, kellyLot);
   kellyLot = MathMin(baseLot * MaxLotMultiplier, kellyLot);
   
   // Equity-based safety
   double maxLotByEquity = equity / 100000.0;
   kellyLot = MathMin(kellyLot, maxLotByEquity);
   
   return ValidateLotSize(kellyLot);
}
...
void SyncPositions()
{
   ...
   // Update Kelly stats when positions close
   CheckForClosedPositions();
}

//+------------------------------------------------------------------+
//| CHECK FOR CLOSED POSITIONS AND UPDATE KELLY STATS                |
//+------------------------------------------------------------------+
void CheckForClosedPositions()
{
   int currentBuyCount = ArraySize(BuySide.positions);
   int currentSellCount = ArraySize(SellSide.positions);
   
   // Detect closed BUY positions
   if(currentBuyCount < lastKnownBuyCount)
   {
      int closedCount = lastKnownBuyCount - currentBuyCount;
      
      // Estimate profit from historical data (positions are already closed)
      // We assume closed positions were profitable (hit TP)
      // This is a simplified approach - in production, track individual positions
      for(int i = 0; i < closedCount; i++)
      {
         UpdateKellyStats("BUY", IndividualTPDollars);  // Assume TP hit
      }
   }
   
   // Detect closed SELL positions  
   if(currentSellCount < lastKnownSellCount)
   {
      int closedCount = lastKnownSellCount - currentSellCount;
      
      for(int i = 0; i < closedCount; i++)
      {
         UpdateKellyStats("SELL", IndividualTPDollars);  // Assume TP hit
      }
   }
   
   // Update counts
   lastKnownBuyCount = currentBuyCount;
   lastKnownSellCount = currentSellCount;
}
...
bool OpenPositionUnified(ENUM_ORDER_TYPE orderType, double price, double lotSize, double levelPrice)
{
   ...
   if(IndividualTPDollars > 0)
   {
      // Check if opposite side is saturated (80%+) based on LOT EXPOSURE
      // This is safer than counting positions - considers actual risk exposure
      double tpTargetDollars = IndividualTPDollars;
      
      double oppositeTotalLots = 0;
      double maxPossibleLots = 0;
      
      if(orderType == ORDER_TYPE_BUY)
      {
         // Opening BUY - check SELL total lot exposure
         for(int i = 0; i < ArraySize(SellSide.positions); i++)
         {
            if(PositionSelectByTicket(SellSide.positions[i].ticket))
            {
               oppositeTotalLots += PositionGetDouble(POSITION_VOLUME);
            }
         }
         // Max possible SELL lots = MaxPositions × MaxLotMultiplier × BaseLot
         maxPossibleLots = MaxPositionsPerSide * MaxLotMultiplier * validatedLotSize;
      }
      else
      {
         // Opening SELL - check BUY total lot exposure
         for(int i = 0; i < ArraySize(BuySide.positions); i++)
         {
            if(PositionSelectByTicket(BuySide.positions[i].ticket))
            {
         ...
         // Max possible BUY lots = MaxPositions × MaxLotMultiplier × BaseLot
         maxPossibleLots = MaxPositionsPerSide * MaxLotMultiplier * validatedLotSize;
      }
      
      // Calculate saturation based on LOT EXPOSURE (not position count)
      double oppositeSaturation = (oppositeTotalLots / maxPossibleLots) * 100.0;
      
      // Calculate current lot multiplier to check if we're scaled
      double lotMultiplier = lotSize / validatedLotSize;
      
      // TP MULTIPLIER: 6x when opposite LOT EXPOSURE >= 80% AND our lot is scaled
      if(oppositeSaturation >= 80.0 && lotMultiplier > 1.0)
      {
         tpTargetDollars = IndividualTPDollars * 6.0;  // 6X TP!
         Print("   🎯 6X TP ACTIVATED!");
         Print("   Opposite lot exposure: ", DoubleToString(oppositeTotalLots, 2), " / ", 
               DoubleToString(maxPossibleLots, 2), " lots (", DoubleToString(oppositeSaturation, 1), "%)");
         Print("   Our lot scaling: ", DoubleToString(lotMultiplier, 1), "x");
...
void PrintDebugStatus()
{
   ...
   Print("Lot Scaling:           ", EnableKellyLotSizing ? "KELLY CRITERION" : "DISABLED");
...
void TogglePanelVisibility()
{
   string objects[] = {
      ...
      "KellyLabel", "BuyWinRate", "SellWinRate", "KellyTrades",
      "RefLabel", "RefPrice",
...
void CreatePanel()
{
   ...
   // Background with increased height for Kelly row
   ...
   ObjectSetInteger(0, panelPrefix + "Background", OBJPROP_YSIZE, 427);  // +22 for Kelly row
   ...
   // === KELLY STATS ROW (if enabled) ===
   if(EnableKellyLotSizing)
   {
      CreateLabel(panelPrefix + "KellyLabel", x + 10, yPos, "Kelly:", clrGold, 9, "Arial Bold");
      CreateLabel(panelPrefix + "BuyWinRate", x + 60, yPos, "B:85%", clrDodgerBlue, 9, "Arial");
      CreateLabel(panelPrefix + "SellWinRate", x + 130, yPos, "S:85%", clrOrangeRed, 9, "Arial");
      CreateLabel(panelPrefix + "KellyTrades", x + 200, yPos, "(0)", clrCyan, 9, "Arial");
      yPos += lineHeight;
   }
   ...
   CreateLabel(panelPrefix + "Equity", x + 70, yPos, "$0", C'230,230,200', 10, "Arial Black");
   yPos += lineHeight;
   ...
   int brandY = y + 427 - 35;  // 35px from bottom (427 = panel height)
...
void UpdatePanel()
{
   ...
   // Equity - Bold chalk color
   double equity = AccountInfoDouble(ACCOUNT_EQUITY);
   ObjectSetString(0, panelPrefix + "Equity", OBJPROP_TEXT, "$" + FormatPrice(equity, 2));
   ObjectSetInteger(0, panelPrefix + "Equity", OBJPROP_COLOR, C'230,230,200');  // Chalk color
   ...
   // Kelly stats (if enabled)
   if(EnableKellyLotSizing)
   {
      int buyWinPct = (int)MathRound(buyStats.winRate * 100);
      int sellWinPct = (int)MathRound(sellStats.winRate * 100);
      int totalKellyTrades = buyStats.totalTrades + sellStats.totalTrades;
      
      ObjectSetString(0, panelPrefix + "BuyWinRate", OBJPROP_TEXT, "B:" + IntegerToString(buyWinPct) + "%");
      ObjectSetString(0, panelPrefix + "SellWinRate", OBJPROP_TEXT, "S:" + IntegerToString(sellWinPct) + "%");
      ObjectSetString(0, panelPrefix + "KellyTrades", OBJPROP_TEXT, "(" + IntegerToString(totalKellyTrades) + ")");
      
      // Color code win rates
      color buyWinColor = (buyWinPct >= 80) ? clrLimeGreen : (buyWinPct >= 70) ? clrYellow : clrOrangeRed;
      color sellWinColor = (sellWinPct >= 80) ? clrLimeGreen : (sellWinPct >= 70) ? clrYellow : clrOrangeRed;
      ObjectSetInteger(0, panelPrefix + "BuyWinRate", OBJPROP_COLOR, buyWinColor);
      ObjectSetInteger(0, panelPrefix + "SellWinRate", OBJPROP_COLOR, sellWinColor);
   }
}
