//+------------------------------------------------------------------+
//|                    TORAMA Aggressive Trader EA v5.2              |
//|                                           TORAMA CAPITAL          |
...
#property description "V5.2: IMPROVED - Existing positions allowed to run to TP/SL on mode switch"
...
void CheckATRModeSwitch()
{
   ...
   // Check if we need to update day's open price (new day)
   MqlDateTime time;
   TimeToStruct(TimeCurrent(), time);
   MqlDateTime lastUpdateTime;
   TimeToStruct(lastDayOpenUpdate, lastUpdateTime);
   
   if(time.day != lastUpdateTime.day)
   {
      UpdateDayOpenPrice();
      // Mode switch cooldown is now time-based, not flag-based
   }
...
void CreatePanel()
{
   ...
   // Background - taller for ATR info
   ...
   ObjectSetInteger(0, panelPrefix + "Background", OBJPROP_YSIZE, 410);  // Increased height
   ...
   // Buttons - Two rows
   CreateButton(panelPrefix + "CloseBtn", x + 10, yPos, 65, 25, "CLOSE", clrRed, clrWhite);
   CreateButton(panelPrefix + "PauseBtn", x + 80, yPos, 65, 25, "PAUSE", clrOrange, clrWhite);
   CreateButton(panelPrefix + "TPBtn", x + 150, yPos, 60, 25, "TP", clrGreen, clrWhite);
   CreateButton(panelPrefix + "SwitchBtn", x + 215, yPos, 55, 25, "MODE", clrDodgerBlue, clrWhite);
   yPos += 35;
   ...
   CreateLabel(panelPrefix + "Direction", x + 100, yPos, CurrentDirection == BUYONLY ? "BUY" : "SELL", dirColor, 9, "Arial Black");
   yPos += lineHeight;
   ...
   if(EnableATRSwitch)
   {
      ...
      CreateLabel(panelPrefix + "ATRValue", x + 100, yPos, "$0", clrAqua, 9, "Arial Bold");
      yPos += lineHeight;
      
      CreateLabel(panelPrefix + "DayOpenLabel", x + 10, yPos, "Day Open:", clrGold, 9, "Arial Bold");
      CreateLabel(panelPrefix + "DayOpenValue", x + 100, yPos, "$0", clrAqua, 9, "Arial Bold");
      yPos += lineHeight;
   }
   ...
   CreateLabel(panelPrefix + "SwitchCount", x + 100, yPos, "0", clrCyan, 9, "Arial Bold");
   yPos += lineHeight + 10;
