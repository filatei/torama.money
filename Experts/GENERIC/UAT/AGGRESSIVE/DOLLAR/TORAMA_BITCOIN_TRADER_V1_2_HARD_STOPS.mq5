...
#property description "V1.2: HARD STOPS (visible SL) - Optimized for $100K accounts"
...
input double   GroupTPDollars = 300.0;                // Group TP ($300 total profit closes all)

// === STOP LOSS - VISIBLE HARD STOPS ===
input group "=== STOP LOSS ==="
input double   IndividualSLDollars = 500.0;           // SL risk per trade ($500 max loss - VISIBLE)
...
int OnInit()
{
   ...
   // Print stop type
   Print("📍 HARD STOPS - Broker SL visible at $", DoubleToString(IndividualSLDollars, 2), " per position");
...
bool OpenPosition(ENUM_ORDER_TYPE orderType, double price, double levelPrice)
{
   ...
   // Calculate TP/SL prices - HARD STOPS (VISIBLE TO BROKER)
   ...
   if(orderType == ORDER_TYPE_BUY)
   {
      ...
      request.sl = price - slDistance;  // HARD STOP - VISIBLE
   }
   else
   {
      request.tp = price - tpDistance;
