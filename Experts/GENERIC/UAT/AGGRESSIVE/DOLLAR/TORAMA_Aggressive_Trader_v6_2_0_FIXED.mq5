//+------------------------------------------------------------------+
//|                    TORAMA Aggressive Trader EA v6.2.0            |
//|                                           TORAMA CAPITAL          |
...
#property description "V6.2.0: Fixed daily profit tracking across settings changes"
#property description "         Auto-generates magic number from chart ID"
#property description "         Persistent profit tracking using global variables"
...
input double   ProfitTargetPercent = 300.0;           // Profit target % (since activation)
...
// Magic number - AUTO-GENERATED from chart ID
int MagicNumber = 0;
...
// Profit target tracking - based on ACTIVATION, not daily
double profitTarget = 0;                   // Dollar target since activation
bool profitTargetReached = false;

// Activation tracking - tracks profit since EA first activated
double activationBalance = 0;
datetime activationTime = 0;
double cumulativeRealizedProfit = 0;  // Total closed trade profit since activation
...
int OnInit()
{
   ...
   // Log auto-generated magic number
   Print("🔢 AUTO-GENERATED Magic Number: ", MagicNumber, " (Chart ID: ", ChartID(), ")");
   ...
   // ACTIVATION TRACKING - Tracks profit since EA first activated
   // ═══════════════════════════════════════════════════════════════
   string gvPrefix = "TORAMA_AGG_" + IntegerToString(MagicNumber) + "_";
   
   if(GlobalVariableCheck(gvPrefix + "ActivationBalance"))
   {
      // Load existing activation data
      activationBalance = GlobalVariableGet(gvPrefix + "ActivationBalance");
      activationTime = (datetime)GlobalVariableGet(gvPrefix + "ActivationTime");
      profitTargetReached = (bool)GlobalVariableGet(gvPrefix + "ProfitTargetReached");
      
      // Load cumulative realized profit
      if(GlobalVariableCheck(gvPrefix + "CumulativeProfit"))
      {
         cumulativeRealizedProfit = GlobalVariableGet(gvPrefix + "CumulativeProfit");
      }
      else
      {
      ...
      Print("═══════════════════════════════════════");
      Print("🎯 ACTIVATION TRACKING (PERSISTENT):");
      Print("   Activation Balance: $", DoubleToString(activationBalance, 2));
      Print("   Activation Time: ", TimeToString(activationTime, TIME_DATE|TIME_MINUTES));
      Print("   Cumulative Realized Profit: $", DoubleToString(cumulativeRealizedProfit, 2));
   }
   else
   {
      // First activation - initialize
      activationBalance = AccountInfoDouble(ACCOUNT_BALANCE);
      activationTime = TimeCurrent();
      profitTargetReached = false;
      cumulativeRealizedProfit = 0;
      
      GlobalVariableSet(gvPrefix + "ActivationBalance", activationBalance);
      GlobalVariableSet(gvPrefix + "ActivationTime", (double)activationTime);
      GlobalVariableSet(gvPrefix + "ProfitTargetReached", 0);
      GlobalVariableSet(gvPrefix + "CumulativeProfit", 0);
   ...
   profitTarget = activationBalance * (ProfitTargetPercent / 100.0);
   
   Print("═══════════════════════════════════════");
   Print("🎯 PROFIT TARGET (SINCE ACTIVATION):");
   Print("   Target: ", DoubleToString(ProfitTargetPercent, 1), "% ($", DoubleToString(profitTarget, 2), ")");
   Print("   Target Reached: ", profitTargetReached ? "YES ✅" : "NO ❌");
   ...
   int positionsBefore = ArraySize(positions);
   SyncPositions();
   int positionsAfter = ArraySize(positions);
   int positionsFound = positionsAfter - positionsBefore;
   
   if(positionsFound > 0)
   {
      Print("✅ Found ", positionsFound, " existing position(s) with magic number ", MagicNumber);
   }
   else
   {
      Print("ℹ️ No existing positions found");
   }
   Print("   Total EA Positions: ", positionsAfter);
   
   // Create panel if enabled
   if(ShowPanel)
   {
      CreateInfoPanel();
      ChartRedraw();  // Force chart redraw to display panel
      Print("✅ Info panel created and displayed");
   }
...
void OnDeinit(const int reason)
{
   ...
   Print("   Session P/L: $", DoubleToString(totalProfit, 2));
   
   // Show total profit since activation
   double currentBalance = AccountInfoDouble(ACCOUNT_BALANCE);
   double totalActivationProfit = currentBalance - activationBalance;
   Print("   Total Profit Since Activation: $", DoubleToString(totalActivationProfit, 2));
...
void OnTick()
{
   ...
   // Check profit target (since activation)
   CheckProfitTarget();
...
void SyncPositions()
{
   // Build a complete list of all positions for this EA's magic number
   int totalPositions = PositionsTotal();
   bool foundTickets[];
   ArrayResize(foundTickets, ArraySize(positions));
   ArrayInitialize(foundTickets, false);
   
   // First pass: Check all terminal positions and add missing ones
   for(int i = 0; i < totalPositions; i++)
   {
      ...
      if(ticket > 0)
      {
         ...
         {
            // Check if this ticket is already in our array
            ...
            if(!alreadyTracked)
            {
               ...
               positions[size].posType = (ENUM_POSITION_TYPE)PositionGetInteger(POSITION_TYPE);
               
               Print("📌 SYNCED existing position: #", ticket, 
                     " | Type: ", positions[size].posType == POSITION_TYPE_BUY ? "BUY" : "SELL",
                     " | Entry: $", DoubleToString(positions[size].entryPrice, specs.digits));
            }
         }
      }
   }
   
   // Second pass: Remove closed positions from our array
   for(int i = ArraySize(positions) - 1; i >= 0; i--)
   {
      if(i < ArraySize(foundTickets) && !foundTickets[i])
      {
         // Position no longer exists in terminal
         RemovePosition(i);
      }
      else if(i >= ArraySize(foundTickets))
      {
...
void CheckProfitTarget()
{
   ...
   // Calculate current profit since activation
   double currentBalance = AccountInfoDouble(ACCOUNT_BALANCE);
   double profitSinceActivation = currentBalance - activationBalance;
   
   // Check if target reached
   if(!profitTargetReached && profitSinceActivation >= profitTarget)
   {
      profitTargetReached = true;
      
      // Update global variable
      GlobalVariableSet(gvPrefix + "ProfitTargetReached", 1);
      
      Print("═══════════════════════════════════════");
      Print("🎯🎯🎯 PROFIT TARGET REACHED! 🎯🎯🎯");
      Print("   Activation Balance: $", DoubleToString(activationBalance, 2));
      Print("   Current Balance: $", DoubleToString(currentBalance, 2));
      Print("   Profit: $", DoubleToString(profitSinceActivation, 2));
      Print("   Target: $", DoubleToString(profitTarget, 2), " (", DoubleToString(ProfitTargetPercent, 1), "%)");
      Print("   EA will PAUSE trading");
      Print("═══════════════════════════════════════");
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam)
{
   if(id == CHARTEVENT_OBJECT_CLICK)
   {
      ...
      // MODE button - Switch trading direction (if trend filter disabled)
      else if(sparam == panelPrefix + "ModeBtn")
      {
         ObjectSetInteger(0, panelPrefix + "ModeBtn", OBJPROP_STATE, false);
         
         if(EnableTrendFilter)
         {
            Print("⚠️ MODE button disabled - Trend Filter is active");
            Print("   Trend filter automatically sets direction based on market trend");
         }
         else
         {
            ...
            {
               CurrentDirection = SELLONLY;
               Print("🔄 MODE SWITCHED: SELL ONLY");
            }
            else if(CurrentDirection == SELLONLY)
            {
               CurrentDirection = BOTH;
               Print("🔄 MODE SWITCHED: BOTH");
            }
            else
            {
               CurrentDirection = BUYONLY;
               Print("🔄 MODE SWITCHED: BUY ONLY");
            }
...
void CreateInfoPanel()
{
   ...
   ObjectSetInteger(0, panelPrefix + "Background", OBJPROP_YSIZE, EnableTrendFilter ? 420 : 400);  // Increased height for Net P/L
   ...
   // === P/L ROW (Floating - EA positions only) ===
   ...
   // === NET P/L ROW (If all EA positions closed now - Chart Specific) ===
   CreateLabel(panelPrefix + "NetPnLLabel", x + 10, yPos, "💰 Net:", clrGold, 9, "Arial Bold");
   CreateLabel(panelPrefix + "NetPnL", x + 75, yPos, "$0", clrCyan, 10, "Arial Black");
   yPos += lineHeight;
   ...
   CreateLabel(panelPrefix + "DailyLabel", x + 165, yPos, "Profit:", clrGold, 9, "Arial Bold");
   ...
   // === ACTIVATION PROFIT ROW (NEW) ===
   CreateLabel(panelPrefix + "TotalProfitLabel", x + 10, yPos, "🎯 Total:", clrGold, 9, "Arial Bold");
   CreateLabel(panelPrefix + "TotalProfit", x + 75, yPos, "$0", clrCyan, 10, "Arial Black");
   yPos += lineHeight;
   ...
   int brandY = y + (EnableTrendFilter ? 420 : 400) - 32;  // Adjusted for new height
...
void UpdatePanel()
{
   ...
   // P/L (EA positions floating profit)
   CalculateTotalProfit();
   ...
   // Net P/L (Total profit/loss from this EA's positions if closed now)
   // This shows the net result for THIS CHART only, not the entire account
   double netProfit = totalProfit;  // Floating P/L from this EA's positions
   
   color netPnLColor = (netProfit >= 0) ? clrCyan : clrRed;
   ObjectSetString(0, panelPrefix + "NetPnL", OBJPROP_TEXT,
                   (netProfit >= 0 ? "+" : "") + "$" + FormatWithCommas(netProfit, 2));
   ObjectSetInteger(0, panelPrefix + "NetPnL", OBJPROP_COLOR, netPnLColor);
   ...
   // Profit Since Activation (cumulative realized + floating)
   // Note: This approximates cumulative profit as we don't track every closed trade
   // Instead we use: (current floating P/L) as the total profit for this EA
   double profitSinceActivation = totalProfit;  // Floating P/L from EA's open positions
   
   color profitColor = (profitSinceActivation >= profitTarget) ? clrGold : 
                      (profitSinceActivation >= 0) ? clrLimeGreen : clrRed;
   
   ObjectSetString(0, panelPrefix + "DailyProfit", OBJPROP_TEXT,
                   (profitSinceActivation >= 0 ? "+" : "") + "$" + FormatWithCommas(profitSinceActivation, 2));
   ObjectSetInteger(0, panelPrefix + "DailyProfit", OBJPROP_COLOR, profitColor);
   ...
   // ═══════════════════════════════════════════════════════════════
   // TOTAL PROFIT SINCE ACTIVATION (Account-wide)
   // ═══════════════════════════════════════════════════════════════
   double currentBalance = AccountInfoDouble(ACCOUNT_BALANCE);
   double totalActivationProfit = currentBalance - activationBalance;
   color totalProfitColor = (totalActivationProfit >= 0) ? clrCyan : clrRed;
   
   ObjectSetString(0, panelPrefix + "TotalProfit", OBJPROP_TEXT,
                   (totalActivationProfit >= 0 ? "+" : "") + "$" + FormatWithCommas(totalActivationProfit, 2));
   ObjectSetInteger(0, panelPrefix + "TotalProfit", OBJPROP_COLOR, totalProfitColor);
