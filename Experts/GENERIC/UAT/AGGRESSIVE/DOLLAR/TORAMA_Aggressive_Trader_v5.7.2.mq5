...
input bool     EnableATRSwitch = false;                // Enable ATR-based mode switching
...
input bool     EnableEquityStop = false;              // Enable equity-based stop
input double   EquityStopLevel = 1000.0;              // Stop trading when equity reaches this $ amount
input bool     EquityStopClosePositions = true;       // Close all positions when equity stop triggers
...
datetime lastModeSwitchTime = 0;           // Last mode switch time
...
int OnInit()
{
   ...
   if(EnableATRSwitch)
   {
      ...
      // Initialize day's open price immediately
      UpdateDayOpenPrice();
   ...
   // Equity stop settings
   if(EnableEquityStop)
   {
      Print("═══════════════════════════════════════");
      Print("💰 EQUITY STOP PROTECTION:");
      Print("   Enabled: YES");
      Print("   Stop Level: $", DoubleToString(EquityStopLevel, 2));
      Print("   Current Equity: $", DoubleToString(AccountInfoDouble(ACCOUNT_EQUITY), 2));
      Print("   Close Positions: ", EquityStopClosePositions ? "YES" : "NO (new trades only)");
      
      if(currentEquity <= EquityStopLevel)
      {
         Print("   ⚠️ WARNING: Current equity already at/below stop level!");
         Print("   EA will trigger equity stop immediately");
      }
      else
      {
         double bufferAmount = currentEquity - EquityStopLevel;
         double bufferPercent = (bufferAmount / currentEquity) * 100.0;
         Print("   Buffer: $", DoubleToString(bufferAmount, 2), " (", DoubleToString(bufferPercent, 1), "%)");
      }
   }
   else
   {
      Print("═══════════════════════════════════════");
      Print("💰 EQUITY STOP PROTECTION: DISABLED");
   }
...
//+------------------------------------------------------------------+
//| CHECK EQUITY STOP LEVEL                                          |
//+------------------------------------------------------------------+
void CheckEquityStop()
{
   if(!EnableEquityStop) return;
   if(equityStopTriggered) return;  // Already triggered
   
   double currentEquity = AccountInfoDouble(ACCOUNT_EQUITY);
   
   // Check if equity has reached or fallen below the stop level
   if(currentEquity <= EquityStopLevel)
   {
      equityStopTriggered = true;
      equityStopReason = StringFormat("Equity reached stop level: $%.2f (Limit: $%.2f)", 
                                      currentEquity, EquityStopLevel);
      
      Print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
      Print("🛑 EQUITY STOP TRIGGERED!");
      Print("   Current Equity: $", DoubleToString(currentEquity, 2));
      Print("   Stop Level: $", DoubleToString(EquityStopLevel, 2));
      Print("   Reason: ", equityStopReason);
      
      // Close positions if configured
      if(EquityStopClosePositions)
      {
      ...
      {
         Print("   ⚠️ Positions NOT closed (EquityStopClosePositions = false)");
         Print("   EA will stop opening new positions only");
      }
...
{
   if(emergencyStop) return;  // Already triggered
   
   double currentEquity = AccountInfoDouble(ACCOUNT_EQUITY);
   
   // Check if equity has fallen to or below the SACROSANCT stop level
   if(currentEquity <= maxDrawdownStopLevel)
   {
      emergencyStop = true;
      emergencyReason = StringFormat("SACROSANCT Max Drawdown Reached: Equity $%.2f ≤ Stop Level $%.2f", 
                                     currentEquity, maxDrawdownStopLevel);
      
      Print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
      Print("🚨🚨🚨 EMERGENCY STOP - MAX DRAWDOWN 🚨🚨🚨");
      Print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
      ...
      Print("   Current Equity: $", DoubleToString(currentEquity, 2));
      Print("   Total Loss: $", DoubleToString(startingBalance - currentEquity, 2));
      Print("   Loss %: ", DoubleToString(((startingBalance - currentEquity) / startingBalance) * 100.0, 2), "%");
      Print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
      Print("   🔴 CLOSING ALL POSITIONS");
      Print("   ⏸️ PAUSING EA - Manual intervention required");
      Print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
      ...
      Print("   ✅ All positions closed");
      Print("   ✅ EA paused");
      Print("   ℹ️ Press RESUME button or restart EA to continue trading");
      Print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
...
void CheckATRModeSwitch()
{
   ...
   // Update ATR value using indicator handle
   if(atrHandle != INVALID_HANDLE)
   {
      double atr_buffer[];
      ArraySetAsSeries(atr_buffer, true);
...
void SwitchTradingMode(ENUM_TRADE_DIRECTION newDirection, string reason)
{
   ...
   // Update grid gap size for new reference price
   currentGapSize = referencePrice * GridGapPercent / 100.0;
...
void PrintDebugStatus()
{
   ...
   if(EnableEquityStop)
   {
      ...
      Print("║ EQUITY STOP                                                  ║");
      Print("╠══════════════════════════════════════════════════════════════╣");
      Print("Equity Stop Level:     $", DoubleToString(EquityStopLevel, 2));
      Print("Equity Stop Status:    ", equityStopTriggered ? "🛑 TRIGGERED" : "✅ Active");
      if(!equityStopTriggered)
      {
         double bufferAmount = equity - EquityStopLevel;
         double bufferPercent = (bufferAmount / equity) * 100.0;
         Print("Buffer to Stop:        $", DoubleToString(bufferAmount, 2), " (", DoubleToString(bufferPercent, 1), "%)");
      }
   }
...
void CreatePanel()
{
   ...
   int panelHeight = 380;
   if(EnableATRSwitch) panelHeight += 66;
   if(EnableTimeFilter) panelHeight += lineHeight;
   if(EnableEquityStop) panelHeight += lineHeight;
   ...
   // === EQUITY STOP ROW (if enabled) ===
   if(EnableEquityStop)
   {
      CreateLabel(panelPrefix + "EqStopLabel", x + 10, yPos, "💰 Eq.Stop@:", clrOrange, 9, "Arial Bold");
      CreateLabel(panelPrefix + "EqStop", x + 95, yPos, "$0", clrOrange, 9, "Arial Bold");
      yPos += lineHeight;
   }
...
void UpdatePanel()
{
   ...
   // Status - prioritize emergency stop, then equity stop
   if(emergencyStop)
   {
      ObjectSetString(0, panelPrefix + "Status", OBJPROP_TEXT, "🚨 DD.STOP");
      ObjectSetInteger(0, panelPrefix + "Status", OBJPROP_COLOR, clrRed);
   }
   else if(equityStopTriggered)
   {
      ObjectSetString(0, panelPrefix + "Status", OBJPROP_TEXT, "🛑 EQ.STOP");
      ObjectSetInteger(0, panelPrefix + "Status", OBJPROP_COLOR, clrRed);
   }
   ...
   // Equity
   ObjectSetString(0, panelPrefix + "Equity", OBJPROP_TEXT, "$" + FormatPrice(currentEquity, 2));
   ...
   // ═══════════════════════════════════════════════════════════════
   string ddTriggerText = "$" + FormatPrice(maxDrawdownStopLevel, 2);
   ...
   // Equity stop display (if enabled)
   if(EnableEquityStop)
   {
      double proximityToStop = ((currentEquity - EquityStopLevel) / EquityStopLevel) * 100;
      
      color eqStopColor = clrOrange;
      if(currentEquity <= EquityStopLevel * 1.05)
         eqStopColor = clrRed;
      else if(currentEquity <= EquityStopLevel * 1.15)
         eqStopColor = clrOrangeRed;
      
      ObjectSetString(0, panelPrefix + "EqStop", OBJPROP_TEXT, "$" + FormatPrice(EquityStopLevel, 2));
      ObjectSetInteger(0, panelPrefix + "EqStop", OBJPROP_COLOR, eqStopColor);
   }
