//+------------------------------------------------------------------+
//|                    TORAMA Universal Grid EA v3.16                |
//|                                           TORAMA CAPITAL          |
...
#property description "v3.16: FIXED - Persistent reference, proper reset logic, unique magic"
...
// v3.16: Auto-generated unique magic number per EA instance
int MagicNumber = 0;
...
double referencePrice = 0;               // v3.16: PERSISTENT reference for grid (never changes except on reset)
...
// v3.16: Enhanced debugging with overflow protection
long debugTickCounter = 0;  // Changed to long to prevent overflow
...
int GenerateUniqueMagicNumber()
{
   ...
   // This ensures each EA activation gets a unique magic number
   datetime currentTime = TimeCurrent();
   MqlDateTime timeStruct;
   TimeToStruct(currentTime, timeStruct);
   
   // Calculate milliseconds component (0-999)
   // Use tick count as proxy for milliseconds
   int milliseconds = (int)(GetTickCount() % 1000);
   
   // Combine: seconds * 1000 + milliseconds
   // Then take modulo to keep within reasonable range (6-7 digits)
   long uniqueValue = (long)currentTime * 1000 + milliseconds;
   int magicNumber = (int)(uniqueValue % 10000000); // Keep it to 7 digits max
...
int OnInit()
{
   ...
   // v3.16: Generate unique magic number per activation
   MagicNumber = GenerateUniqueMagicNumber();
   Print("🔑 UNIQUE MAGIC NUMBER: ", MagicNumber);
   Print("   (Auto-generated from activation timestamp)");
   ...
   // v3.16: Initialize PERSISTENT reference price
   // This reference remains constant unless explicitly reset
   ...
   Print("📌 REFERENCE PRICE: $", DoubleToString(referencePrice, 2), " (PERSISTENT - never changes)");
   ...
   Print("First Position Opens: When price moves ±1 full grid level from reference");
   ...
   Print("═══════════════════════════════════════");
   Print("🔍 v3.16 IMPROVEMENTS:");
   Print("   ✅ Reference price now PERSISTENT across mode changes");
   Print("   ✅ Multiplier resets on ALL-CLOSE and MODE-CHANGE");
   Print("   ✅ Unique magic number per EA activation");
   Print("   ✅ Debug counter overflow protection (long type)");
   Print("   ✅ Fixed lot size logging in position opening");
...
void OnTick()
{
   ...
   // Reset counter periodically to prevent eventual overflow (every 1 million ticks)
   if(debugTickCounter > 1000000)
      debugTickCounter = 0;
   ...
   // v3.16: Check if all positions closed - ALWAYS reset multiplier, optionally return to NEUTRAL
   ...
   if(lastPositionCount > 0 && currentPositionCount == 0)
   {
      ...
      // v3.16: ALWAYS reset multiplier when all positions close
      ...
      if(ReturnToNeutralOnAllClose && currentMode != MODE_NEUTRAL)
      {
         ...
         // v3.16: DO NOT change reference price - it remains persistent
         // Only reset water marks and tracking variables
         highWaterMark = currentPrice;
         lowWaterMark = currentPrice;
         levelsAgainstTrend = 0;
         lastProcessedLevel = currentPrice;
         ...
         Print("   📌 Reference: $", DoubleToString(referencePrice, 2), " (UNCHANGED - persistent)");
   ...
   // NEUTRAL MODE: Wait for price to hit first grid level (full gap from persistent reference)
   if(currentMode == MODE_NEUTRAL && ArraySize(positions) == 0)
   {
      // v3.16: Enhanced debug logging every 100 ticks
      static long neutralTickCounter = 0;
      neutralTickCounter++;
      
      if(debugVerbose && neutralTickCounter % 100 == 0)
      {
         ...
         Print("   📌 Reference: $", DoubleToString(referencePrice, 2), " (PERSISTENT)");
      ...
      if(currentPrice >= referencePrice + currentGapSize)
      {
         ...
         // v3.16: Reference price remains unchanged - it's persistent!
         trendStartPrice = currentPrice;
         highWaterMark = currentPrice;
         levelsAgainstTrend = 0;
         lastProcessedLevel = currentPrice;
         ...
         Print("   📌 Reference: $", DoubleToString(referencePrice, 2), " (UNCHANGED - persistent)");
         Print("   Moved: $", DoubleToString(currentPrice - referencePrice, 2));
      ...
      else if(currentPrice <= referencePrice - currentGapSize)
      {
         ...
         Print("   📌 Reference: $", DoubleToString(referencePrice, 2), " (UNCHANGED - persistent)");
         Print("   Moved: $", DoubleToString(referencePrice - currentPrice, 2));
   ...
   if(currentMode == MODE_BUY)
   {
      ...
      if(currentPrice < lastProcessedLevel - currentGapSize)
      {
         ...
         if(levelsAgainstTrend >= LevelsBeforeFlip)
         {
            ...
            currentMode = MODE_SELL;
            // v3.16: Reference price NEVER changes - it's persistent across mode changes
            trendStartPrice = currentPrice;
            lowWaterMark = currentPrice;
            levelsAgainstTrend = 0;
            lastProcessedLevel = currentPrice;
            ...
            // v3.16: Reset multiplier on mode change if enabled
   ...
   {
      // v3.16: Debug when max positions reached
      if(debugVerbose && debugTickCounter % 200 == 0)
      {
...
void CheckGridLevels()
{
   ...
   {
      // First position in current mode - check distance from reference
      shouldOpen = true;
   }
   else
   {
...
bool OpenPosition(ENUM_ORDER_TYPE type, double price)
{
   ...
   if(!OrderSend(request, result))
   {
      ...
      Print("   Volume: ", lotToUse);  // v3.16: Fixed - now shows actual lot used
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam)
{
   ...
   if(id == CHARTEVENT_OBJECT_CLICK)
   {
      if(sparam == panelPrefix + "SwitchBtn")
      {
         ...
         // v3.16: Cycle through NEUTRAL → BUY → SELL → NEUTRAL
         double currentPrice = (SymbolInfoDouble(_Symbol, SYMBOL_ASK) + SymbolInfoDouble(_Symbol, SYMBOL_BID)) / 2.0;
         ...
         if(currentMode == MODE_NEUTRAL)
         {
            ...
            Print("   📌 Reference: $", DoubleToString(referencePrice, 2), " (UNCHANGED)");
         }
      ...
      else if(sparam == panelPrefix + "CloseAllBtn")
      {
         ...
         if(ArraySize(positions) > 0)
         {
            ...
            // v3.16: Return to NEUTRAL after closing all, but keep persistent reference
            double currentPrice = (SymbolInfoDouble(_Symbol, SYMBOL_ASK) + SymbolInfoDouble(_Symbol, SYMBOL_BID)) / 2.0;
            currentMode = MODE_NEUTRAL;
            highWaterMark = currentPrice;
            lowWaterMark = currentPrice;
...
void PrintDebugStatus()
{
   ...
   Print("╔══════════════════════════════════════════════════════════════╗");
   Print("║              TORAMA GRID EA - DEBUG STATUS v3.16             ║");
   Print("╠══════════════════════════════════════════════════════════════╣");
   ...
   Print("📌 Reference Price:    $", DoubleToString(referencePrice, (int)SymbolInfoInteger(_Symbol, SYMBOL_DIGITS)), " (PERSISTENT)");
   ...
   Print("║ v3.16 IMPROVEMENTS                                           ║");
   Print("╠══════════════════════════════════════════════════════════════╣");
   Print("✅ Reference price is PERSISTENT across all mode changes");
   Print("✅ Multiplier resets on ALL-CLOSE and MODE-CHANGE");
   Print("✅ Unique magic number per EA activation: ", MagicNumber);
   Print("✅ Debug counter overflow protection (long type)");
   Print("✅ Fixed lot size logging in position opening");
   Print("╚══════════════════════════════════════════════════════════════╝");
...
void CreatePanel()
{
   ...
   // Grid Reference Price - Base price for all grid calculations (PERSISTENT)
   ...
   // v3.16: LOT SIZE with MULTIPLIER (bottom left area)
...
void UpdatePanel()
{
   ...
   // Grid Reference Price - Shows the PERSISTENT base price from which all grid levels are calculated
   ...
   // v3.16: Update lot size and multiplier display
